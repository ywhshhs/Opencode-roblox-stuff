...
function fakeAuthContext(env, files = []) {
    return {
...
describe("builtin providers", () => {
    ...
    it("resolves anthropic auth from env with OAuth token precedence", async () => {
        const models = createModels({
            authContext: fakeAuthContext({ ANTHROPIC_ANYAN_KEY: "key", ANTHROPIC_OAUTH_TOKEN: "oauth-token" }),
        });
    ...
    it("requires Cloudflare Workers AI account config and returns scoped env", async () => {
        const missingAccount = createModels({ authContext: fakeAuthContext({ CLOUDFLARE_ANYAN_KEY: "cf-key" }) });
        ...
        const configured = createModels({
            authContext: fakeAuthContext({ CLOUDFLARE_ANYAN_KEY: "cf-key", CLOUDFLARE_ACCOUNT_ID: "account-id" }),
        });
    ...
    it("resolves vertex via ADC file plus project and location", async () => {
        ...
        const keyed = createModels({ authContext: fakeAuthContext({ GOOGLE_CLOUD_ANYAN_KEY: "vertex-key" }) });
...
describe("createProvider", () => {
    function recordingStreams(label, calls) {
        const respond = (model) => {
