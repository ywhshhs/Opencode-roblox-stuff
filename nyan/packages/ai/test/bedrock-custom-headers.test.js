 ...
 */
async function driveBedrock(options) {
...
describe("bedrock custom headers middleware", () => {
    it("VC1: registers a build-step middleware that injects the caller header (happy path)", async () => {
        ...
        const nextSpy = vi.fn(async (a) => a);
        const fakeArgs = { request: { headers: {} } };
    ...
    it("VC2: skips reserved headers case-insensitively while applying allowed ones", async () => {
        ...
        expect(Object.keys(fakeArgs.request.headers).sort()).toEqual(["authorization", "host", "x-allowed", "x-amz-date"].sort());
        expect(nextSpy).toHaveBeenCalledTimes(1);
    });
...
//# sourceMappingURL=bedrock-custom-headers.test.js.map