...
const originalGoogleCloudApiKey = process.env.GOOGLE_CLOUD_ANYAN_KEY;
beforeEach(() => {
    googleGenAiMock.constructorCalls.length = 0;
    delete process.env.GOOGLE_CLOUD_ANYAN_KEY;
});
afterEach(() => {
    if (originalGoogleCloudApiKey === undefined) {
        ...
        process.env.GOOGLE_CLOUD_ANYAN_KEY = originalGoogleCloudApiKey;
    }
});
describe("google-vertex api key resolution", () => {
    ...
    it("falls back to ADC when GOOGLE_CLOUD_ANYAN_KEY is a placeholder marker", async () => {
        process.env.GOOGLE_CLOUD_ANYAN_KEY = "<authenticated>";
        const stream = streamGoogleVertex(model, context, {
            project: "test-project",
            location: "us-central1",
        });
    ...
    it("forwards custom baseUrl to the ADC client", async () => {
        const customModel = { ...model, baseUrl: "https://proxy.example.com" };
...
//# sourceMappingURL=google-vertex-api-key-resolution.test.js.map