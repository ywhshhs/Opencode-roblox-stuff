...
function generateOverflowContent(contextWindow) {
...
async function testContextOverflow(model, apiKey) {
...
describe("Context overflow error handling", () => {
    describe.skipIf(!process.env.ANTHROPIC_ANYAN_KEY)("Anthropic (API Key)", () => {
        it("claude-haiku-4-5 - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.ANTHROPIC_ANYAN_KEY);
            logResult(result);
    ...
    describe("GitHub Copilot (OAuth)", () => {
        ...
        it.skipIf(!githubCopilotToken)("gemini-2.5-pro - should detect overflow via isContextOverflow", async () => {
        ...
        it.skipIf(!githubCopilotToken)("claude-sonnet-4 - should detect overflow via isContextOverflow", async () => {
    ...
    describe.skipIf(!process.env.OPENAI_ANYAN_KEY)("OpenAI Completions", () => {
        it("gpt-4o-mini - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.OPENAI_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
    ...
    describe.skipIf(!hasAzureOpenAICredentials())("Azure OpenAI Responses", () => {
        it("gpt-4o-mini - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.AZURE_OPENAI_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
    ...
    describe.skipIf(!process.env.GEMINI_ANYAN_KEY)("Google", () => {
        it("gemini-2.0-flash - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.GEMINI_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
    ...
    describe("OpenAI Codex (OAuth)", () => {
        it.skipIf(!openaiCodexToken)("gpt-5.5 - should detect overflow via isContextOverflow", async () => {
    ...
    describe.skipIf(!process.env.XAI_ANYAN_KEY)("xAI", () => {
        it("grok-3-fast - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.XAI_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
    ...
    describe.skipIf(!process.env.GROQ_ANYAN_KEY)("Groq", () => {
        it("llama-3.3-70b-versatile - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.GROQ_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
    ...
    describe.skipIf(!process.env.CEREBRAS_ANYAN_KEY)("Cerebras", () => {
        it("available model - should detect overflow via isContextOverflow", async () => {
            const preferredCerebrasModelIds = ["gpt-oss-120b", "zai-glm-4.7", "llama3.1-8b"];
            const cerebrasModels = getModels("cerebras");
            const model = cerebrasModels.find((candidate) => preferredCerebrasModelIds.includes(candidate.id)) ?? cerebrasModels[0];
            if (!model) {
                throw new Error("No Cerebras models available");
            }
            const result = await testContextOverflow(model, process.env.CEREBRAS_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
    ...
    describe.skipIf(!process.env.TOGETHER_ANYAN_KEY)("Together AI", () => {
        it("Kimi-K2.6 - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.TOGETHER_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
            expect(isContextOverflow(result.response, model.contextWindow)).toBe(true);
        }, 120000);
    ...
    describe.skipIf(!process.env.ZAI_ANYAN_KEY)("z.ai", () => {
        it("glm-4.5-air - should detect overflow via isContextOverflow when z.ai reports it", async () => {
            ...
            const result = await testContextOverflow(model, process.env.ZAI_ANYAN_KEY);
            logResult(result);
    ...
    describe.skipIf(!process.env.MISTRAL_ANYAN_KEY)("Mistral", () => {
        it("devstral-medium-latest - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.MISTRAL_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
    ...
    describe.skipIf(!process.env.MINIMAX_ANYAN_KEY)("MiniMax", () => {
        it("MiniMax-M2.7 - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.MINIMAX_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
            expect(isContextOverflow(result.response, model.contextWindow)).toBe(true);
        }, 120000);
    ...
    describe.skipIf(!process.env.XIAOMI_ANYAN_KEY)("Xiaomi MiMo (API billing)", () => {
        ...
        it("mimo-v2.5-pro - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.XIAOMI_ANYAN_KEY);
            logResult(result);
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_CN_ANYAN_KEY)("Xiaomi MiMo Token Plan (CN)", () => {
        it("mimo-v2.5-pro - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.XIAOMI_TOKEN_PLAN_CN_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("length");
            expect(result.usage.output).toBe(0);
            expect(isContextOverflow(result.response, model.contextWindow)).toBe(true);
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_AMS_ANYAN_KEY)("Xiaomi MiMo Token Plan (AMS)", () => {
        it("mimo-v2.5-pro - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.XIAOMI_TOKEN_PLAN_AMS_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("length");
            expect(result.usage.output).toBe(0);
            expect(isContextOverflow(result.response, model.contextWindow)).toBe(true);
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_SGP_ANYAN_KEY)("Xiaomi MiMo Token Plan (SGP)", () => {
        it("mimo-v2.5-pro - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.XIAOMI_TOKEN_PLAN_SGP_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("length");
            expect(result.usage.output).toBe(0);
            expect(isContextOverflow(result.response, model.contextWindow)).toBe(true);
    ...
    describe.skipIf(!process.env.KIMI_ANYAN_KEY)("Kimi For Coding", () => {
        it("kimi-k2-thinking - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.KIMI_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
            expect(isContextOverflow(result.response, model.contextWindow)).toBe(true);
        }, 120000);
    ...
    describe.skipIf(!process.env.AI_GATEWAY_ANYAN_KEY)("Vercel AI Gateway", () => {
        it("google/gemini-2.5-flash via AI Gateway - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.AI_GATEWAY_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
            expect(isContextOverflow(result.response, model.contextWindow)).toBe(true);
        }, 120000);
    ...
    describe.skipIf(!process.env.OPENROUTER_ANYAN_KEY)("OpenRouter", () => {
        ...
        it("anthropic/claude-sonnet-4 via OpenRouter - should detect overflow via isContextOverflow", async () => {
            ...
            const result = await testContextOverflow(model, process.env.OPENROUTER_ANYAN_KEY);
            logResult(result);
            expect(result.stopReason).toBe("error");
    ...
    if (!process.env.NYAN_NO_LOCAL_LLM) {
        try {
            ...
            const probeStatus = execSync('curl -s --max-time 1 -o /dev/null -w \'%{http_code}\' -X POST http://localhost:8081/v1/completions -H \'content-type: application/json\' -d \'{"model":"local-model","prompt":"ping","max_tokens":1}\'', { encoding: "utf8" }).trim();
