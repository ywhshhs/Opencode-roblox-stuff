...
describe("Cache Retention (NYAN_CACHE_RETENTION)", () => {
	...
	describe("Anthropic Provider", () => {
		it.skipIf(!process.env.ANTHROPIC_ANYAN_KEY)(
			"should use default cache TTL (no ttl field) when NYAN_CACHE_RETENTION is not set",
			async () => {
	...
	describe("OpenAI Responses Provider", () => {
		it.skipIf(!process.env.OPENAI_ANYAN_KEY)(
			"should not set prompt_cache_retention when NYAN_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.OPENAI_ANYAN_KEY)(
			"should set prompt_cache_retention to 24h when NYAN_CACHE_RETENTION=long",
			async () => {
				process.env.NYAN_CACHE_RETENTION = "long";
				const model = getModel("openai", "gpt-4o-mini");
				let capturedPayload: any = null;
