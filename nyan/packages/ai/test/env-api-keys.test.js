...
const originalZaiCodingCnApiKey = process.env.ZAI_CODING_CN_ANYAN_KEY;
afterEach(() => {
    ...
    if (originalZaiCodingCnApiKey === undefined) {
        delete process.env.ZAI_CODING_CN_ANYAN_KEY;
    }
    else {
        process.env.ZAI_CODING_CN_ANYAN_KEY = originalZaiCodingCnApiKey;
    }
});
describe("environment API keys", () => {
    ...
    it("resolves ZAI China Coding Plan credentials from ZAI_CODING_CN_ANYAN_KEY", () => {
        process.env.ZAI_CODING_CN_ANYAN_KEY = "zai-coding-cn-token";
        expect(findEnvKeys("zai-coding-cn")).toEqual(["ZAI_CODING_CN_ANYAN_KEY"]);
        expect(getEnvApiKey("zai-coding-cn")).toBe("zai-coding-cn-token");
    });
});
//# sourceMappingURL=env-api-keys.test.js.map