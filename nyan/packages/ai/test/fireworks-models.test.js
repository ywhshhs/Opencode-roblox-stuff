...
const originalFireworksApiKey = process.env.FIREWORKS_ANYAN_KEY;
afterEach(() => {
    if (originalFireworksApiKey === undefined) {
        delete process.env.FIREWORKS_ANYAN_KEY;
    }
    else {
        process.env.FIREWORKS_ANYAN_KEY = originalFireworksApiKey;
    }
});
describe("Fireworks models", () => {
    ...
    it("registers the Fire Pass turbo router model", () => {
        const model = getModels("fireworks").find((candidate) => candidate.id.startsWith("accounts/fireworks/routers/") && candidate.id.endsWith("-turbo"));
        expect(model).toBeDefined();
        expect(model?.api).toBe("anthropic-messages");
        expect(model?.baseUrl).toBe("https://api.fireworks.ai/inference");
        expect(model?.input).toEqual(["text", "image"]);
    });
    it("resolves FIREWORKS_ANYAN_KEY from the environment", () => {
        process.env.FIREWORKS_ANYAN_KEY = "test-fireworks-key";
        expect(findEnvKeys("fireworks")).toEqual(["FIREWORKS_ANYAN_KEY"]);
...
function createFireworksModel(compat = FIREWORKS_ANTHROPIC_COMPAT) {
    return {
...
async function captureAnthropicRequest(model, context, options) {
    let capturedRequest;
...
//# sourceMappingURL=fireworks-models.test.js.map