...
describe.skipIf(!process.env.OPENAI_ANYAN_KEY || !process.env.ANTHROPIC_ANYAN_KEY)("OpenAI Responses reasoning replay e2e", () => {
    it("skips reasoning-only history after an aborted turn", { retry: 2 }, async () => {
        const model = getModel("openai", "gpt-5-mini");
        const apiKey = getEnvApiKey("openai");
        if (!apiKey) {
            throw new Error("Missing OPENAI_ANYAN_KEY");
        }
        const userMessage = {
            role: "user",
        ...
        const assistantResponse = await complete(model, {
        ...
        const thinkingBlock = assistantResponse.content.find((block) => block.type === "thinking" && block.thinkingSignature);
    ...
    it("handles same-provider different-model handoff with tool calls", { retry: 2 }, async () => {
        ...
        const assistantResponse = await complete(modelA, {
        ...
        const toolCallBlock = assistantResponse.content.find((block) => block.type === "toolCall");
        if (!toolCallBlock) {
    ...
    it("handles cross-provider handoff from Anthropic to OpenAI Codex", { retry: 2 }, async () => {
        ...
        const assistantResponse = await complete(anthropicModel, {
            systemPrompt: "You are a helpful assistant. Always use the tool when asked.",
            messages: [userMessage],
            tools: [testTool],
        }, {
        ...
        if (functionCalls.length > 0) {
            console.log("- function_call IDs:", functionCalls.map((fc) => fc.id));
        }
...
//# sourceMappingURL=openai-responses-reasoning-replay-e2e.test.js.map