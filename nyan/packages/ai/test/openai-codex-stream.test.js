...
import { getOpenAICodexWebSocketDebugStats, resetOpenAICodexWebSocketDebugStats, stream as streamOpenAICodexResponses, streamSimple as streamSimpleOpenAICodexResponses, } from "../src/api/openai-codex-responses.js";
const originalAgentDir = process.env.NYAN_CODING_AGENT_DIR;
afterEach(() => {
    vi.unstubAllGlobals();
    if (originalAgentDir === undefined) {
        delete process.env.NYAN_CODING_AGENT_DIR;
    }
    else {
        process.env.NYAN_CODING_AGENT_DIR = originalAgentDir;
    }
...
function mockToken() {
    const payload = Buffer.from(JSON.stringify({ "https://api.openai.com/auth": { chatgpt_account_id: "acc_test" } }), "utf8").toString("base64");
    return `aaa.${payload}.bbb`;
}
function buildSSEPayload({ status, includeDone = false, }) {
...
describe("openai-codex streaming", () => {
    it("streams SSE responses into AssistantMessageEventStream", async () => {
        ...
        process.env.NYAN_CODING_AGENT_DIR = tempDir;
        const payload = Buffer.from(JSON.stringify({ "https://api.openai.com/auth": { chatgpt_account_id: "acc_test" } }), "utf8").toString("base64");
    ...
    ])("uses the client-sent %s service tier for %s when Codex echoes default", async (modelId, serviceTier, multiplier) => {
        const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
        process.env.NYAN_CODING_AGENT_DIR = tempDir;
        const token = mockToken();
        const sse = `${[
    ...
    it("reconnects once when the websocket connection limit is reached before output starts", async () => {
        ...
        class MockWebSocket extends EventTarget {
            constructor() {
                super();
                this.limitReached = connections++ === 0;
        ...
        const result = await streamOpenAICodexResponses(model, { systemPrompt: "", messages: [] }, {
            apiKey: token,
        }).result();
        expect(result.stopReason).toBe("stop");
    ...
    ])("uses %s for SSE retries", async (_name, makeHeaders, expectedDelay) => {
        vi.useFakeTimers();
    ...
    it("uses exponential backoff across repeated SSE retries without retry headers", async () => {
        ...
        const retryTimeoutDelays = () => setTimeoutSpy.mock.calls
            .map((call) => call[1])
            .filter((delay) => delay === 1000 || delay === 2000 || delay === 4000);
        const resultPromise = streamOpenAICodexResponses(model, context, {
            apiKey: token,
            transport: "sse",
            maxRetries: 3,
...
//# sourceMappingURL=openai-codex-stream.test.js.map