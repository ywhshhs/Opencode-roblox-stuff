...
async function captureSimpleParams(model, reasoning) {
    let payload;
...
describe("openai-completions tool_choice", () => {
    ...
    it("forwards toolChoice from simple options to payload", async () => {
        ...
        const model = { ...baseModel, api: "openai-completions" };
        const tools = [
            {
                name: "ping",
        ...
        const params = (payload ?? mockState.lastParams);
    ...
    it("accumulates mixed content, reasoning, and parallel tool call deltas independently", async () => {
        ...
        const eventTypes = [];
        const toolEventsByContentIndex = new Map();
        for await (const event of s) {
    ...
    it("stores Xiaomi MiMo reasoning replay compat in built-in metadata", () => {
        const providers = ["xiaomi", "xiaomi-token-plan-cn", "xiaomi-token-plan-ams", "xiaomi-token-plan-sgp"];
    ...
    it("sends max_tokens for OpenCode completions models", async () => {
        const cases = [getModel("opencode-go", "kimi-k2.6"), getModel("opencode", "grok-build-0.1")];
        for (const model of cases) {
            let payload;
    ...
    it("omits Ant Ling reasoning for unmapped direct reasoning efforts and non-reasoning models", async () => {
        ...
        expect((payload ?? mockState.lastParams)).not.toHaveProperty("reasoning");
        const ling = getModel("ant-ling", "Ling-2.6-flash");
        await streamSimple(ling, {
            messages: [{ role: "user", content: "Hi", timestamp: Date.now() }],
        }, {
            apiKey: "test",
            reasoning: "high",
...
//# sourceMappingURL=openai-completions-tool-choice.test.js.map