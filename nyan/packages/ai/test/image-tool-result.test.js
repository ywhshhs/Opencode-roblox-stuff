 ...
 */
async function handleToolWithImageResult(model, options) {
 ...
 */
async function handleToolWithTextAndImageResult(model, options) {
    // Check if the model supports images
    if (!model.input.includes("image")) {
    ...
    const context = {
        ...
        messages: [
            {
                role: "user",
                content: "Use the get_circle_with_description tool and tell me what you learned. Also say what color the shape is.",
                timestamp: Date.now(),
            },
        ],
        tools: [getImageTool],
...
describe("Tool Results with Images", () => {
    describe.skipIf(!process.env.GEMINI_ANYAN_KEY)("Google Provider (gemini-2.5-flash)", () => {
    ...
    describe.skipIf(!process.env.OPENAI_ANYAN_KEY)("OpenAI Responses Provider (gpt-5-mini)", () => {
    ...
    describe.skipIf(!process.env.OPENROUTER_ANYAN_KEY)("OpenRouter Provider (glm-4.5v)", () => {
    ...
    describe.skipIf(!process.env.MISTRAL_ANYAN_KEY)("Mistral Provider (pixtral-12b)", () => {
    ...
    describe.skipIf(!process.env.TOGETHER_ANYAN_KEY)("Together AI Provider (Kimi-K2.6)", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_ANYAN_KEY)("Xiaomi MiMo (API billing) Provider (mimo-v2.5-pro)", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_CN_ANYAN_KEY)("Xiaomi MiMo Token Plan (CN) Provider (mimo-v2.5-pro)", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_AMS_ANYAN_KEY)("Xiaomi MiMo Token Plan (AMS) Provider (mimo-v2.5-pro)", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_SGP_ANYAN_KEY)("Xiaomi MiMo Token Plan (SGP) Provider (mimo-v2.5-pro)", () => {
    ...
    describe.skipIf(!process.env.KIMI_ANYAN_KEY)("Kimi For Coding Provider (kimi-for-coding)", () => {
    ...
    describe.skipIf(!process.env.AI_GATEWAY_ANYAN_KEY)("Vercel AI Gateway Provider (google/gemini-2.5-flash)", () => {
    ...
    describe("Anthropic OAuth Provider (claude-sonnet-4-5)", () => {
        ...
        it.skipIf(!anthropicOAuthToken)("should handle tool result with only image", { retry: 3, timeout: 30000 }, async () => {
            await handleToolWithImageResult(model, { apiKey: anthropicOAuthToken });
        });
        it.skipIf(!anthropicOAuthToken)("should handle tool result with text and image", { retry: 3, timeout: 30000 }, async () => {
            await handleToolWithTextAndImageResult(model, { apiKey: anthropicOAuthToken });
        });
    });
    describe("GitHub Copilot Provider", () => {
        it.skipIf(!githubCopilotToken)("claude-haiku-4.5 - should handle tool result with only image", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-haiku-4.5");
            await handleToolWithImageResult(llm, { apiKey: githubCopilotToken });
        });
        it.skipIf(!githubCopilotToken)("claude-haiku-4.5 - should handle tool result with text and image", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-haiku-4.5");
            await handleToolWithTextAndImageResult(llm, { apiKey: githubCopilotToken });
        });
        it.skipIf(!githubCopilotToken)("claude-sonnet-4 - should handle tool result with only image", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-sonnet-4.6");
            await handleToolWithImageResult(llm, { apiKey: githubCopilotToken });
        });
        it.skipIf(!githubCopilotToken)("claude-sonnet-4 - should handle tool result with text and image", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-sonnet-4.6");
            await handleToolWithTextAndImageResult(llm, { apiKey: githubCopilotToken });
        });
    });
    describe("OpenAI Codex Provider", () => {
        it.skipIf(!openaiCodexToken)("gpt-5.5 - should handle tool result with only image", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("openai-codex", "gpt-5.5");
            await handleToolWithImageResult(llm, { apiKey: openaiCodexToken });
        });
        it.skipIf(!openaiCodexToken)("gpt-5.5 - should handle tool result with text and image", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("openai-codex", "gpt-5.5");
            await handleToolWithTextAndImageResult(llm, { apiKey: openaiCodexToken });
        });
    });
});
//# sourceMappingURL=image-tool-result.test.js.map