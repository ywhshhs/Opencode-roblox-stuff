...
async function captureOpenAIResponseHeaders(options, model = getModel("openai", "gpt-5.4")) {
    const captured = { sessionId: null, clientRequestId: null };
...
describe("openai-responses provider defaults", () => {
    ...
    it.each(["gpt-5.1", "gpt-5.2", "gpt-5.3-codex", "gpt-5.4", "gpt-5.4-mini", "gpt-5.4-nano", "gpt-5.5"])("sends none reasoning effort for OpenAI %s when no reasoning is requested", async (modelId) => {
    ...
    it.each(["gpt-5", "gpt-5-mini", "gpt-5-nano", "gpt-5-pro", "gpt-5.2-pro", "gpt-5.4-pro", "gpt-5.5-pro"])("omits reasoning effort for OpenAI %s when off is unsupported", async (modelId) => {
        const model = getModel("openai", modelId);
        let capturedPayload;
        vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response("data: [DONE]\n\n", {
            status: 200,
    ...
    it("clamps prompt_cache_key to OpenAI's 64-character limit", async () => {
        ...
        const stream = streamOpenAIResponses(getModel("openai", "gpt-5.4"), {
            systemPrompt: "sys",
            messages: [{ role: "user", content: "hi", timestamp: Date.now() }],
        }, {
            apiKey: "test-key",
    ...
    it.each([
        ...
        ["gpt-5.5", "flex", 0.5],
    ])("applies %s %s service-tier cost multiplier", async (modelId, serviceTier, multiplier) => {
        const model = getModel("openai", modelId);
        const sse = `${[
            `data: ${JSON.stringify({
...
//# sourceMappingURL=openai-responses-copilot-provider.test.js.map