...
const originalTogetherApiKey = process.env.TOGETHER_ANYAN_KEY;
afterEach(() => {
    if (originalTogetherApiKey === undefined) {
        delete process.env.TOGETHER_ANYAN_KEY;
    }
    else {
        process.env.TOGETHER_ANYAN_KEY = originalTogetherApiKey;
    }
});
describe("Together models", () => {
    ...
    it("resolves TOGETHER_ANYAN_KEY from the environment", () => {
        process.env.TOGETHER_ANYAN_KEY = "test-together-key";
        expect(findEnvKeys("together")).toEqual(["TOGETHER_ANYAN_KEY"]);
        expect(getEnvApiKey("together")).toBe("test-together-key");
    });
});
//# sourceMappingURL=together-models.test.js.map