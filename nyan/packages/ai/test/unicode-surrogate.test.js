 ...
 */
async function testEmojiInToolResults(llm, options = {}) {
...
async function testRealWorldLinkedInData(llm, options = {}) {
...
async function testUnpairedHighSurrogate(llm, options = {}) {
...
describe("AI Providers Unicode Surrogate Pair Tests", () => {
    describe.skipIf(!process.env.GEMINI_ANYAN_KEY)("Google Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.OPENAI_ANYAN_KEY)("OpenAI Completions Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.OPENAI_ANYAN_KEY)("OpenAI Responses Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.ANTHROPIC_ANYAN_KEY)("Anthropic Provider Unicode Handling", () => {
    ...
    describe("Anthropic OAuth Provider Unicode Handling", () => {
        ...
        it.skipIf(!anthropicOAuthToken)("should handle real-world LinkedIn comment data with emoji", { retry: 3, timeout: 30000 }, async () => {
            await testRealWorldLinkedInData(llm, { apiKey: anthropicOAuthToken });
        });
        it.skipIf(!anthropicOAuthToken)("should handle unpaired high surrogate (0xD83D) in tool results", { retry: 3, timeout: 30000 }, async () => {
            await testUnpairedHighSurrogate(llm, { apiKey: anthropicOAuthToken });
        });
    });
    describe("GitHub Copilot Provider Unicode Handling", () => {
        it.skipIf(!githubCopilotToken)("claude-haiku-4.5 - should handle emoji in tool results", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-haiku-4.5");
            await testEmojiInToolResults(llm, { apiKey: githubCopilotToken });
        });
        it.skipIf(!githubCopilotToken)("claude-haiku-4.5 - should handle real-world LinkedIn comment data with emoji", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-haiku-4.5");
            await testRealWorldLinkedInData(llm, { apiKey: githubCopilotToken });
        });
        it.skipIf(!githubCopilotToken)("claude-haiku-4.5 - should handle unpaired high surrogate (0xD83D) in tool results", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-haiku-4.5");
            await testUnpairedHighSurrogate(llm, { apiKey: githubCopilotToken });
        });
        it.skipIf(!githubCopilotToken)("claude-sonnet-4 - should handle emoji in tool results", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-sonnet-4.6");
            await testEmojiInToolResults(llm, { apiKey: githubCopilotToken });
        });
        it.skipIf(!githubCopilotToken)("claude-sonnet-4 - should handle real-world LinkedIn comment data with emoji", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-sonnet-4.6");
            await testRealWorldLinkedInData(llm, { apiKey: githubCopilotToken });
        });
        it.skipIf(!githubCopilotToken)("claude-sonnet-4 - should handle unpaired high surrogate (0xD83D) in tool results", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("github-copilot", "claude-sonnet-4.6");
            await testUnpairedHighSurrogate(llm, { apiKey: githubCopilotToken });
        });
    });
    describe.skipIf(!process.env.XAI_ANYAN_KEY)("xAI Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.GROQ_ANYAN_KEY)("Groq Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.CEREBRAS_ANYAN_KEY)("Cerebras Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.TOGETHER_ANYAN_KEY)("Together AI Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.ZAI_ANYAN_KEY)("zAI Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.MISTRAL_ANYAN_KEY)("Mistral Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.MINIMAX_ANYAN_KEY)("MiniMax Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_ANYAN_KEY)("Xiaomi MiMo (API billing) Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_CN_ANYAN_KEY)("Xiaomi MiMo Token Plan (CN) Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_AMS_ANYAN_KEY)("Xiaomi MiMo Token Plan (AMS) Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_SGP_ANYAN_KEY)("Xiaomi MiMo Token Plan (SGP) Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.KIMI_ANYAN_KEY)("Kimi For Coding Provider Unicode Handling", () => {
    ...
    describe.skipIf(!process.env.AI_GATEWAY_ANYAN_KEY)("Vercel AI Gateway Provider Unicode Handling", () => {
    ...
    describe("OpenAI Codex Provider Unicode Handling", () => {
        it.skipIf(!openaiCodexToken)("gpt-5.5 - should handle emoji in tool results", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("openai-codex", "gpt-5.5");
            await testEmojiInToolResults(llm, { apiKey: openaiCodexToken });
        });
        it.skipIf(!openaiCodexToken)("gpt-5.5 - should handle real-world LinkedIn comment data with emoji", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("openai-codex", "gpt-5.5");
            await testRealWorldLinkedInData(llm, { apiKey: openaiCodexToken });
        });
        it.skipIf(!openaiCodexToken)("gpt-5.5 - should handle unpaired high surrogate (0xD83D) in tool results", { retry: 3, timeout: 30000 }, async () => {
            const llm = getModel("openai-codex", "gpt-5.5");
            await testUnpairedHighSurrogate(llm, { apiKey: openaiCodexToken });
        });
    });
});
//# sourceMappingURL=unicode-surrogate.test.js.map