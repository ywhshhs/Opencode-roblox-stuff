...
import { closeOpenAICodexWebSocketSessions, getOpenAICodexWebSocketDebugStats, resetOpenAICodexWebSocketDebugStats, stream as streamOpenAICodexResponses, } from "../src/api/openai-codex-responses.js";
...
async function main() {
    ...
    const context = {
        systemPrompt: "You are participating in a benchmark. For each benchmark turn, call deterministic_probe exactly once before the final answer. Keep final answers minimal.",
        messages: [],
    ...
    console.log(`turns ${args.turns}, transport ${args.transport}, reasoning ${args.reasoning}, maxTokens ${args.maxTokens}`);
    ...
    for (let turn = 1; turn <= args.turns; turn++) {
        ...
        while (true) {
            ...
            for (const call of toolCalls) {
                context.messages.push(executeTool(call));
                toolResults++;
            }
            if (requests > 4)
                throw new Error(`Too many requests for turn ${turn}`);
        }
...
//# sourceMappingURL=codex-websocket-cached-probe.js.map