...
import { loginOpenAICodexDeviceCode, openaiCodexOAuthProvider, refreshOpenAICodexToken, } from "../src/utils/oauth/openai-codex.js";
...
function deviceAuthPendingResponse() {
    return jsonResponse({
        error: {
...
describe("OpenAI Codex OAuth", () => {
    ...
    it("cancels the OpenAI Codex device code flow while waiting", async () => {
        ...
        const rejectionPromise = credentialsPromise.then(() => new Error("Expected login to fail"), (error) => error);
        for (let i = 0; i < 5 && pollTimes.length === 0; i++) {
            await vi.advanceTimersByTimeAsync(0);
        }
        ...
        expect(rejection.message).toBe("Login cancelled");
    });
    it("times out the OpenAI Codex device code flow after 15 minutes", async () => {
        ...
        const rejection = await rejectionPromise;
        expect(rejection).toBeInstanceOf(Error);
        expect(rejection.message).toBe("Device flow timed out");
    });
    ...
    it("includes the response body in OpenAI Codex device auth poll failures", async () => {
        ...
        await expect(loginOpenAICodexDeviceCode({
            onDeviceCode: () => { },
        })).rejects.toThrow('OpenAI Codex device auth failed with status 500: {"error":"server_error","error_description":"try again later"}');
    });
    it("does not write token refresh failures to stderr", async () => {
        ...
        await expect(refreshOpenAICodexToken("invalid-refresh-token")).rejects.toThrow(/OpenAI Codex token refresh failed \(401\).*Could not validate your token/);
        expect(consoleError).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=openai-codex-oauth.test.js.map