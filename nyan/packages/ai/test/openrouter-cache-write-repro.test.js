...
function createLongSystemPrompt() {
    const nonce = `${Date.now()}-${Math.random()}`;
    return `You are a concise assistant.\nCache nonce: ${nonce}\n\n${Array(80)
        .fill("Prompt-caching probe content. Keep this exact text stable across requests so the provider can reuse prefix tokens and report cache read and cache write usage.")
        .join("\n\n")}`;
}
describe.skipIf(!process.env.OPENROUTER_ANYAN_KEY)("OpenRouter cache_write repro E2E", () => {
    it("regression: preserves cache_write_tokens on openai-completions stream path", { retry: 2, timeout: 90000 }, async () => {
        ...
        const options = {
            apiKey: process.env.OPENROUTER_ANYAN_KEY,
            maxTokens: 32,
            temperature: 0,
            onPayload: (payload) => {
...
//# sourceMappingURL=openrouter-cache-write-repro.test.js.map