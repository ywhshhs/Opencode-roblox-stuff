...
import { complete, fauxAssistantMessage, fauxText, fauxThinking, fauxToolCall, registerFauxProvider, stream, Type, } from "../src/compat.js";
async function collectEvents(streamResult) {
    const events = [];
...
describe("faux provider", () => {
    ...
    it("emits an error when a response factory throws", async () => {
        ...
        const events = await collectEvents(stream(registration.getModel(), { messages: [{ role: "user", content: "hi", timestamp: Date.now() }] }));
    ...
    it("supports aborting before the first chunk", async () => {
        ...
        controller.abort();
        const events = await collectEvents(stream(registration.getModel(), { messages: [{ role: "user", content: "hi", timestamp: Date.now() }] }, { signal: controller.signal }));
        expect(events).toHaveLength(1);
        expect(events[0].type).toBe("error");
        if (events[0].type === "error") {
    ...
    it("supports aborting mid-text stream when paced", async () => {
        ...
        const s = stream(registration.getModel(), { messages: [{ role: "user", content: "hi", timestamp: Date.now() }] }, { signal: controller.signal });
        for await (const event of s) {
            events.push(event.type);
    ...
    it("unregisters the provider", async () => {
        ...
        registration.setResponses([fauxAssistantMessage("hello")]);
        registration.unregister();
        await expect(complete(registration.getModel(), { messages: [{ role: "user", content: "hi", timestamp: Date.now() }] })).rejects.toThrow(`No API provider registered for api: ${registration.api}`);
    });
});
//# sourceMappingURL=faux-provider.test.js.map