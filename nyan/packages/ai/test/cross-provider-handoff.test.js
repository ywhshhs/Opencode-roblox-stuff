...
const PROVIDER_MODEL_PAIRS = [
    ...
    },
    {
        ...
        upstreamApiKeyEnv: "ANTHROPIC_ANYAN_KEY",
    },
    {
        provider: "cloudflare-ai-gateway",
        model: "gpt-5.1",
        label: "cloudflare-gateway-gpt-5.1",
        upstreamApiKeyEnv: "OPENAI_ANYAN_KEY",
    },
    // Groq
...
function getHeaders(pair) {
    if (!pair.upstreamApiKeyEnv)
        return undefined;
 ...
 */
async function generateContext(pair, apiKey) {
    const baseModel = getModel(pair.provider, pair.model);
    if (!baseModel) {
        console.log(`  Model not found: ${pair.provider}/${pair.model}`);
        return null;
    }
    const model = pair.apiOverride ? { ...baseModel, api: pair.apiOverride } : baseModel;
    const userMessage = {
        role: "user",
    ...
    try {
        assistantResponse = await completeSimple(model, {
    ...
    try {
        finalResponse = await completeSimple(model, {
...
describe.skipIf(!hasAnyApiKey())("Cross-Provider Handoff", () => {
    ...
    it.skipIf(!hasAnyApiKey())("should handle cross-provider handoffs for each target", async () => {
        ...
        for (const targetPair of availablePairs) {
            ...
            const allMessages = [
                ...
                {
                    role: "user",
                    content: "Great, thanks for all that help! Now just say 'Hello, handoff successful!' to confirm you received everything.",
                    timestamp: Date.now(),
                },
            ];
            const baseModel = getModel(targetPair.provider, targetPair.model);
            if (!baseModel) {
                console.log(`[Target: ${targetPair.label}] Model not found`);
                continue;
            }
            const model = targetPair.apiOverride
...
//# sourceMappingURL=cross-provider-handoff.test.js.map