...
async function basicTextGeneration(model, options) {
    const context = {
...
async function handleToolCall(model, options) {
    const context = {
...
async function handleStreaming(model, options) {
...
async function handleThinking(model, options) {
...
async function handleImage(model, options) {
...
async function multiTurn(model, options) {
    const context = {
...
describe("Generate E2E Tests", () => {
    describe.skipIf(!process.env.GEMINI_ANYAN_KEY)("Gemini Provider (gemini-2.5-flash)", () => {
    ...
    describe.skipIf(!process.env.OPENAI_ANYAN_KEY)("OpenAI Completions Provider (gpt-4o-mini)", () => {
    ...
    describe.skipIf(!process.env.DEEPSEEK_ANYAN_KEY)("DeepSeek Provider (deepseek-v4-flash via OpenAI Completions)", () => {
    ...
    describe.skipIf(!process.env.OPENAI_ANYAN_KEY)("OpenAI Responses Provider (gpt-5.4)", () => {
    ...
    describe.skipIf(!process.env.ANTHROPIC_ANYAN_KEY)("Anthropic Provider (claude-haiku-4-5)", () => {
    ...
    describe.skipIf(!process.env.XAI_ANYAN_KEY)("xAI Provider (grok-code-fast-1 via OpenAI Completions)", () => {
    ...
    describe.skipIf(!process.env.GROQ_ANYAN_KEY)("Groq Provider (gpt-oss-20b via OpenAI Completions)", () => {
    ...
    describe.skipIf(!process.env.CEREBRAS_ANYAN_KEY)("Cerebras Provider (gpt-oss-120b via OpenAI Completions)", () => {
    ...
    describe.skipIf(!hasCloudflareWorkersAICredentials())("Cloudflare Workers AI Provider (Kimi K2.6 via OpenAI Completions)", () => {
    ...
    describe.skipIf(!hasCloudflareAiGatewayCredentials())("Cloudflare AI Gateway → Workers AI (Kimi K2.6 via /compat)", () => {
    ...
    describe.skipIf(!hasCloudflareAiGatewayCredentials() || !process.env.OPENAI_ANYAN_KEY)("Cloudflare AI Gateway → OpenAI BYOK (gpt-5.1 via /openai responses)", () => {
        const llm = getModel("cloudflare-ai-gateway", "gpt-5.1");
        const options = { headers: { Authorization: `Bearer ${process.env.OPENAI_ANYAN_KEY}` } };
    ...
    describe.skipIf(!hasCloudflareAiGatewayCredentials() || !process.env.ANTHROPIC_ANYAN_KEY)("Cloudflare AI Gateway → Anthropic BYOK (claude-sonnet-4-5 via /anthropic messages)", () => {
        const llm = getModel("cloudflare-ai-gateway", "claude-sonnet-4-5");
        const options = { headers: { Authorization: `Bearer ${process.env.ANTHROPIC_ANYAN_KEY}` } };
        const thinkingOptions = {
            ...options,
            thinkingEnabled: true,
    ...
    describe.skipIf(!process.env.TOGETHER_ANYAN_KEY)("Together AI Provider (Kimi-K2.6 via OpenAI Completions)", () => {
    ...
    describe.skipIf(!process.env.NVIDIA_ANYAN_KEY)("NVIDIA NIM Provider (Nemotron 3 Super via OpenAI Completions)", () => {
    ...
    describe.skipIf(!process.env.OPENROUTER_ANYAN_KEY)("OpenRouter Provider (glm-4.5v via OpenAI Completions)", () => {
    ...
    describe.skipIf(!process.env.AI_GATEWAY_ANYAN_KEY)("Vercel AI Gateway Provider (google/gemini-2.5-flash via Anthropic Messages)", () => {
    ...
    describe.skipIf(!process.env.AI_GATEWAY_ANYAN_KEY)("Vercel AI Gateway Provider (anthropic/claude-opus-4.5 via Anthropic Messages)", () => {
    ...
    describe.skipIf(!process.env.AI_GATEWAY_ANYAN_KEY)("Vercel AI Gateway Provider (openai/gpt-5.1-codex-max via Anthropic Messages)", () => {
    ...
    describe.skipIf(!process.env.ZAI_ANYAN_KEY)("zAI Provider (glm-5.1 via OpenAI Completions)", () => {
    ...
    describe.skipIf(!process.env.MISTRAL_ANYAN_KEY)("Mistral Provider (devstral-medium-latest)", () => {
    ...
    describe.skipIf(!process.env.MISTRAL_ANYAN_KEY)("Mistral Provider (pixtral-12b with image support)", () => {
    ...
    describe.skipIf(!process.env.MINIMAX_ANYAN_KEY)("MiniMax Provider (MiniMax-M2.7 via Anthropic Messages)", () => {
    ...
    describe.skipIf(!process.env.KIMI_ANYAN_KEY)("Kimi For Coding Provider (kimi-k2-thinking via Anthropic Messages)", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_ANYAN_KEY)("Xiaomi MiMo (API billing) Provider (Xiaomi MiMo-V2.5-Pro via Anthropic Messages)", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_CN_ANYAN_KEY)("Xiaomi MiMo Token Plan Provider (Xiaomi MiMo-V2.5-Pro via Anthropic Messages, CN region)", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_AMS_ANYAN_KEY)("Xiaomi MiMo Token Plan Provider (Xiaomi MiMo-V2.5-Pro via Anthropic Messages, AMS region)", () => {
    ...
    describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_SGP_ANYAN_KEY)("Xiaomi MiMo Token Plan Provider (Xiaomi MiMo-V2.5-Pro via Anthropic Messages, SGP region)", () => {
    ...
    describe.skipIf(!process.env.ANT_LING_ANYAN_KEY)("Ant Ling Provider (Ling 2.6 Flash via OpenAI Completions)", () => {
    ...
    describe("Anthropic OAuth Provider (claude-opus-4-6 with adaptive thinking)", () => {
        ...
        it.skipIf(!anthropicOAuthToken)("should handle multi-turn with adaptive thinking and tools", { retry: 3 }, async () => {
    ...
    describe("OpenAI Codex Provider (gpt-5.5 via WebSocket)", () => {
        const llm = getModel("openai-codex", "gpt-5.5");
        const wsOptions = { apiKey: openaiCodexToken, transport: "websocket" };
        it.skipIf(!openaiCodexToken)("should complete basic text generation", { retry: 3 }, async () => {
    ...
    describe.skipIf(!hasBedrockCredentials())("Amazon Bedrock Provider (claude-opus-4-6 interleaved thinking)", () => {
        ...
        it("should use adaptive thinking without anthropic_beta", { retry: 3 }, async () => {
            let capturedPayload;
            const response = await complete(llm, {
                systemPrompt: "You are a helpful assistant that uses tools when asked.",
                messages: [
                    {
                        role: "user",
        ...
        it("should pass requestMetadata to the SDK payload", { retry: 3 }, async () => {
            ...
            const response = await complete(llmSonnet, {
                messages: [
                    {
                        role: "user",
                        content: "Say hi.",
            ...
            expect(response.stopReason, `Error: ${response.errorMessage}`).not.toBe("error");
            expect(capturedPayload).toBeTruthy();
            expect(capturedPayload.requestMetadata).toEqual(metadata);
        });
        it("should omit requestMetadata from payload when not provided", { retry: 3 }, async () => {
            ...
            expect(response.stopReason, `Error: ${response.errorMessage}`).not.toBe("error");
            expect(capturedPayload).toBeTruthy();
            expect("requestMetadata" in capturedPayload).toBe(false);
        });
    });
    // Check if ollama is installed and local LLM tests are enabled
    let ollamaInstalled = false;
    if (!process.env.NYAN_NO_LOCAL_LLM) {
        try {
