...
import { CLOUDFLARE_AI_GATEWAY_ANTHROPIC_BASE_URL, CLOUDFLARE_AI_GATEWAY_COMPAT_BASE_URL, CLOUDFLARE_AI_GATEWAY_OPENAI_BASE_URL, CLOUDFLARE_WORKERS_AI_BASE_URL, } from "../src/api/cloudflare.js";
...
const TOGETHER_TOGGLE_REASONING_COMPAT = {
    ...TOGETHER_BASE_COMPAT,
    thinkingFormat: "together",
};
const TOGETHER_REASONING_EFFORT_COMPAT = {
    ...TOGETHER_BASE_COMPAT,
    supportsReasoningEffort: true,
    thinkingFormat: "openai",
};
const TOGETHER_TOGGLE_REASONING_EFFORT_COMPAT = {
...
function mergeThinkingLevelMap(model, map) {
    model.thinkingLevelMap = { ...model.thinkingLevelMap, ...map };
}
function getTogetherCompat(modelId, reasoning) {
    if (!reasoning)
...
function getTogetherThinkingLevelMap(modelId, reasoning) {
    if (!reasoning)
        return undefined;
    if (TOGETHER_REASONING_EFFORT_MODELS.has(modelId))
...
function supportsOpenAiXhigh(modelId) {
    return (modelId.includes("gpt-5.2") ||
        modelId.includes("gpt-5.3") ||
        modelId.includes("gpt-5.4") ||
        modelId.includes("gpt-5.5"));
}
function isGoogleThinkingApi(model) {
...
function isAnthropicTemperatureUnsupportedModel(modelId) {
...
function mergeAnthropicMessagesCompat(model, compat) {
...
function openAICompletionsCompatDelta(compat) {
    const delta = {};
    for (const [key, value] of Object.entries(compat)) {
        const defaultValue = OPENAI_COMPLETIONS_DEFAULT_COMPAT[key];
        if (isPlainEmptyObject(value) && isPlainEmptyObject(defaultValue))
            continue;
...
function mergeOpenAICompletionsCompat(model, compat) {
    model.compat = { ...model.compat, ...compat };
}
function applyOpenAICompletionsCompatMetadata(model) {
...
function applyThinkingLevelMetadata(model) {
    ...
    if (model.api === "openai-completions" && model.id.includes("deepseek-v4")) {
        mergeThinkingLevelMap(model, model.provider === "openrouter"
...
function getAnthropicMessagesCompat(provider, modelId) {
    const compat = {};
...
function getBedrockBaseUrl(modelId) {
...
async function fetchNvidiaNimModelIds() {
    try {
...
async function loadModelsDevData() {
    try {
        ...
        const data = await response.json();
        const models = [];
        const nvidiaNimModelIds = data.nvidia?.models ? await fetchNvidiaNimModelIds() : new Map();
        ...
        if (data["cloudflare-ai-gateway"]?.models) {
            for (const [prefixedId, model] of Object.entries(data["cloudflare-ai-gateway"].models)) {
                ...
                const compat = upstream === "anthropic" || upstream === "workers-ai" ? { sendSessionAffinityHeaders: true } : undefined;
                models.push({
                    id,
                    name: m.name || id,
                    api,
        ...
        if (data.nvidia?.models) {
            for (const [modelId, model] of Object.entries(data.nvidia.models)) {
                ...
                if (m.tool_call !== true)
                    continue;
                if (!m.modalities?.input?.includes("text"))
                    continue;
                if (!m.modalities?.output?.includes("text"))
                    continue;
                const liveModelId = nvidiaNimModelIds.get(modelId) ?? nvidiaNimModelIds.get(normalizeNvidiaModelId(modelId));
                if (!liveModelId)
                    continue;
                if (NVIDIA_NIM_UNSUPPORTED_MODELS.has(liveModelId))
                    continue;
                models.push({
                    id: liveModelId,
        ...
        if (data["github-copilot"]?.models) {
            for (const [modelId, model] of Object.entries(data["github-copilot"].models)) {
                ...
                const api = isCopilotClaude4
                    ? "anthropic-messages"
                    : needsResponsesApi
                        ? "openai-responses"
                        : "openai-completions";
                const anthropicCompat = api === "anthropic-messages" ? getAnthropicMessagesCompat("github-copilot", modelId) : undefined;
                const copilotModel = {
                    id: modelId,
                    name: m.name || modelId,
                    api,
        ...
        if (data["kimi-for-coding"]?.models) {
            ...
            for (const [modelId, model] of Object.entries(kimiModels)) {
                ...
                if (kimiAliases.has(modelId) && hasCanonicalModel)
                    continue;
        ...
        const getMoonshotProviderModels = (key) => {
        ...
        if (data.xiaomi?.models) {
            for (const { provider, baseUrl } of xiaomiVariants) {
                for (const [modelId, model] of Object.entries(data.xiaomi.models)) {
                    ...
                    if (m.tool_call !== true)
                        continue;
                    if (provider.startsWith("xiaomi-token-plan-") && modelId === "mimo-v2-flash")
                        continue;
                    models.push({
                        id: modelId,
                        name: m.name || modelId,
...
async function generateModels() {
    ...
    const allModels = [...modelsDevModels, ...openRouterModels, ...aiGatewayModels].filter((model) => !((model.provider === "opencode" || model.provider === "opencode-go") && model.id === "gpt-5.3-codex-spark"));
    ...
    const azureOpenAiModels = allModels
...
`;
    const catalogConstName = (providerId) => `${providerId.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_MODELS`;
    function emitModel(model, indent) {
