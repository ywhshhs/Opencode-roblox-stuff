...
async function fetchOpenRouterImageModels() {
    try {
        ...
        for (const model of data.data ?? []) {
            const input = Array.from(new Set((model.architecture?.input_modalities ?? [])
                .filter((modality) => modality === "text" || modality === "image")));
            const output = Array.from(new Set((model.architecture?.output_modalities ?? []).filter((modality) => modality === "text" || modality === "image")));
...
function generateImageModelsFile(models) {
    const imageModelsByProvider = {
        openrouter: Object.fromEntries(models
...
//# sourceMappingURL=generate-image-models.js.map