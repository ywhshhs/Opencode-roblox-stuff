...
```bash
npm install @nyan-works/nyan-ai
```

TypeBox exports are re-exported from `@nyan-works/nyan-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@nyan-works/nyan-ai';
import { builtinModels } from '@nyan-works/nyan-ai/providers/all';
...
```typescript
import { anthropicProvider } from '@nyan-works/nyan-ai/providers/anthropic';
import { openaiProvider } from '@nyan-works/nyan-ai/providers/openai';
import { openrouterProvider } from '@nyan-works/nyan-ai/providers/openrouter';
import { amazonBedrockProvider } from '@nyan-works/nyan-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@nyan-works/nyan-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@nyan-works/nyan-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@nyan-works/nyan-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@nyan-works/nyan-ai';
...
```typescript
import { validateToolCall, type Tool } from '@nyan-works/nyan-ai';
...
```typescript
import { builtinImagesModels } from '@nyan-works/nyan-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@nyan-works/nyan-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@nyan-works/nyan-ai';
import { openAICompletionsApi } from '@nyan-works/nyan-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@nyan-works/nyan-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@nyan-works/nyan-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@nyan-works/nyan-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@nyan-works/nyan-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@nyan-works/nyan-ai';
import { anthropicProvider } from '@nyan-works/nyan-ai/providers/anthropic';
import { openaiProvider } from '@nyan-works/nyan-ai/providers/openai';
import { googleProvider } from '@nyan-works/nyan-ai/providers/google';
...
```typescript
import { createModels } from '@nyan-works/nyan-ai';
import { anthropicProvider } from '@nyan-works/nyan-ai/providers/anthropic';
...
- `@nyan-works/nyan-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@nyan-works/nyan-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@nyan-works/nyan-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@nyan-works/nyan-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@nyan-works/nyan-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@nyan-works/nyan-ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @nyan-works/nyan-ai login              # interactive provider selection
npx @nyan-works/nyan-ai login anthropic    # login to specific provider
npx @nyan-works/nyan-ai list               # list available providers
```
...
The legacy flow functions remain available via the `@nyan-works/nyan-ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@nyan-works/nyan-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@nyan-works/nyan-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@nyan-works/nyan-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@nyan-works/nyan-ai`.
