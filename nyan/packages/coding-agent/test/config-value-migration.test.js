...
describe("config value env var syntax migration", () => {
    ...
    it("leaves uppercase auth.json API key values unchanged", () => {
        ...
        fs.writeFileSync(path.join(agentDir, "auth.json"), `${JSON.stringify({
            anthropic: { type: "api_key", key: "ANTHROPIC_ANYAN_KEY" },
            openai: { type: "api_key", key: "$OPENAI_ANYAN_KEY" },
        ...
        const migrated = JSON.parse(fs.readFileSync(path.join(agentDir, "auth.json"), "utf-8"));
        expect(migrated.anthropic.key).toBe("ANTHROPIC_ANYAN_KEY");
        expect(migrated.openai.key).toBe("$OPENAI_ANYAN_KEY");
    ...
    it("leaves uppercase models.json API key and header values unchanged", async () => {
        const agentDir = createAgentDir();
        const envKeys = ["CUSTOM_ANYAN_KEY", "HEADER_ANYAN_KEY", "MODEL_ANYAN_KEY", "OVERRIDE_ANYAN_KEY"];
        const savedEnv = {};
        ...
        try {
            fs.writeFileSync(path.join(agentDir, "models.json"), `${JSON.stringify({
                providers: {
                    "custom-provider": {
                        ...
                        headers: {
                            "x-api-key": "HEADER_ANYAN_KEY",
                            "x-literal": "literal",
                        },
                        models: [
                            {
                                id: "model-a",
                                headers: { "x-model-key": "MODEL_ANYAN_KEY" },
                            },
                        ],
                        modelOverrides: {
                            "model-b": { headers: { "x-override-key": "OVERRIDE_ANYAN_KEY" } },
                        },
                    },
                },
            }, null, 2)}\n`, "utf-8");
            ...
            expect(provider.apiKey).toBe("CUSTOM_ANYAN_KEY");
            expect(provider.headers?.["x-api-key"]).toBe("HEADER_ANYAN_KEY");
            expect(provider.headers?.["x-literal"]).toBe("literal");
            expect(provider.models?.[0]?.headers?.["x-model-key"]).toBe("MODEL_ANYAN_KEY");
            expect(provider.modelOverrides?.["model-b"]?.headers?.["x-override-key"]).toBe("OVERRIDE_ANYAN_KEY");
            expect(logSpy).not.toHaveBeenCalled();
            ...
            expect(await registry.getApiKeyForProvider("custom-provider")).toBe("CUSTOM_ANYAN_KEY");
...
//# sourceMappingURL=config-value-migration.test.js.map