...
describe("package commands", () => {
    ...
    function getNewerPatchVersion() {
        const [major = "0", minor = "0", patch = "0"] = VERSION.split(".");
        return `${major}.${minor}.${Number.parseInt(patch, 10) + 1}`;
    }
    async function runPackageCommandDirectly(args) {
    ...
    beforeEach(() => {
        ...
        originalPiPackageDir = process.env.NYAN_PACKAGE_DIR;
    ...
    it("should remove local packages using a path with a trailing slash", async () => {
        ...
        const settingsPath = join(agentDir, "settings.json");
        const installedSettings = JSON.parse(readFileSync(settingsPath, "utf-8"));
        expect(installedSettings.packages?.length).toBe(1);
        await main(["remove", `${packageDir}/`]);
        const removedSettings = JSON.parse(readFileSync(settingsPath, "utf-8"));
    ...
    it("does not prompt or ask extensions for project trust during update", async () => {
        ...
        writeFileSync(fakeNpmPath, `const fs=require("node:fs");fs.writeFileSync(${JSON.stringify(recordPath)},JSON.stringify(process.argv.slice(2)));`);
        writeFileSync(join(projectDir, ".pi", "settings.json"), JSON.stringify({ packages: ["npm:fake-package"], npmCommand: [originalExecPath, fakeNpmPath] }));
        let projectTrustCalled = false;
        const logSpy = vi.spyOn(console, "log").mockImplementation(() => { });
        try {
            await expect(main(["update", "--extensions"], {
                extensionFactories: [
                    (pi) => {
                        pi.on("project_trust", () => {
    ...
    it("uses the update check version for forced self updates even when current", async () => {
        ...
        mkdirSync(join(projectDir, ".pi"), { recursive: true });
        writeFileSync(fakeNpmPath, `const fs=require("node:fs"),path=require("node:path"),args=process.argv.slice(2),prefix=args[args.indexOf("--prefix")+1];
...
`);
        writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ npmCommand: [originalExecPath, fakeNpmPath, "--prefix", globalPrefix] }, null, 2));
        writeFileSync(join(projectDir, ".pi", "settings.json"), JSON.stringify({ npmCommand: [originalExecPath, fakeNpmPath, "--prefix", projectPrefix] }, null, 2));
        process.env.NYAN_PACKAGE_DIR = selfPackageDir;
        ...
        try {
            ...
            const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
            const recordedArgs = JSON.parse(readFileSync(recordPath, "utf-8"));
...
`);
        ...
        vi.stubGlobal("fetch", vi.fn(async () => Response.json({ packageName: activePackageName, version: "0.73.0" })));
        const logSpy = vi.spyOn(console, "log").mockImplementation(() => { });
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        try {
            await expect(runPackageCommandDirectly(["update", "--self"])).resolves.toBeUndefined();
            expect(process.exitCode).toBeUndefined();
            expect(errorSpy).not.toHaveBeenCalled();
            const recordedCalls = JSON.parse(readFileSync(recordPath, "utf-8"));
...
//# sourceMappingURL=package-command-paths.test.js.map