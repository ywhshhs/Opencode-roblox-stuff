...
function getContrast(rgb, bgLum) {
    const fgLum = getLuminance(...rgb);
    const lighter = Math.max(fgLum, bgLum);
    const darker = Math.min(fgLum, bgLum);
    return (lighter + 0.05) / (darker + 0.05);
}
function adjustColorToContrast(hex, targetContrast, againstWhite) {
...
const reset = "\x1b[0m";
// --- Commands ---
function cmdContrast(targetContrast) {
    const baseColors = {
        teal: "#5f8787",
        blue: "#5f87af",
        green: "#87af87",
...
function cmdTest(filePath) {
    ...
    for (const [name, hex] of Object.entries(vars)) {
        ...
        console.log(`${name.padEnd(14)} ${fgAnsi(hex)}Sample text${reset}  ${hex}  white: ${vsWhite.toFixed(2)}:1 ${passW.padEnd(5)}  black: ${vsBlack.toFixed(2)}:1 ${passB}`);
    }
}
function cmdTheme(themeName) {
    process.env.COLORTERM = "truecolor";
    initTheme(themeName);
    const parseAnsiRgb = (ansi) => {
        const match = ansi.match(/38;2;(\d+);(\d+);(\d+)/);
        return match ? [parseInt(match[1], 10), parseInt(match[2], 10), parseInt(match[3], 10)] : null;
    };
    const getContrastVsWhite = (colorName) => {
    ...
    const getContrastVsBlack = (colorName) => {
        const ansi = theme.getFgAnsi(colorName);
        const rgb = parseAnsiRgb(ansi);
        if (!rgb)
            return "(default)";
    ...
    const logColor = (name) => {
        const sample = theme.fg(name, "Sample text");
...
//# sourceMappingURL=test-theme-colors.js.map