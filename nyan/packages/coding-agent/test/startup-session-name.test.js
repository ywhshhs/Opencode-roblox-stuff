...
function createSessionFile(projectDir, sessionFile) {
    const timestamp = new Date().toISOString();
    writeFileSync(sessionFile, `${JSON.stringify({ type: "session", version: 3, id: "existing-session", timestamp, cwd: projectDir })}\n${JSON.stringify({
        type: "message",
        id: "assistant-1",
        parentId: null,
        timestamp,
...
function readSessionInfoNames(sessionFile) {
...
async function runCli(args, dirs) {
    let stderr = "";
...
describe("startup session name", () => {
    it("sets --name on the selected session before runtime model validation", async () => {
        const dirs = setup();
        const result = await runCli(["--session", dirs.sessionFile, "--name", "  CLI Named Session  ", "--model", "missing-model", "-p", "hi"], dirs);
    ...
    it("rejects empty --name values without appending session metadata", async () => {
        const dirs = setup();
        const result = await runCli(["--session", dirs.sessionFile, "--name", "   ", "--model", "missing-model", "-p", "hi"], dirs);
        expect(result.code).toBe(1);
        expect(result.signal).toBeNull();
...
//# sourceMappingURL=startup-session-name.test.js.map