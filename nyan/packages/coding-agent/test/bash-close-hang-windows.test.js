...
function toBashSingleQuotedArg(value) {
    return `'${value.replace(/\\/g, "/").replace(/'/g, `'"'"'`)}'`;
}
function createInheritedStdioCommand(pidFile) {
...
function cleanupDetachedChild(pidFile) {
...
describe.skipIf(process.platform !== "win32")("Windows child-process close handling", () => {
    ...
    it("executeBash resolves after the shell exits even if inherited stdio handles stay open", async () => {
        ...
        try {
            const result = await withTimeout(executeBashWithOperations(command, process.cwd(), createLocalBashOperations(), {
...
//# sourceMappingURL=bash-close-hang-windows.test.js.map