...
describe("Package Manager git source parsing", () => {
    ...
    describe("protocol URLs without git: prefix", () => {
        ...
        it("should parse ssh:// URL", () => {
            const parsed = packageManager.parseSource("ssh://git@github.com/user/repo");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("github.com");
            expect(parsed.path).toBe("user/repo");
    ...
    describe("shorthand URLs with git: prefix", () => {
        it("should parse git@host:path format", () => {
            const parsed = packageManager.parseSource("git:git@github.com:user/repo");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("github.com");
            expect(parsed.path).toBe("user/repo");
        ...
        it("should parse shorthand with ref", () => {
            const parsed = packageManager.parseSource("git:git@github.com:user/repo@v1.0.0");
            expect(parsed.type).toBe("git");
    ...
    describe("identity normalization", () => {
        it("should normalize protocol and shorthand-prefixed URLs to same identity", () => {
            const prefixed = packageManager.getPackageIdentity("git:git@github.com:user/repo");
            const https = packageManager.getPackageIdentity("https://github.com/user/repo");
            const ssh = packageManager.getPackageIdentity("ssh://git@github.com/user/repo");
...
//# sourceMappingURL=package-manager-ssh.test.js.map