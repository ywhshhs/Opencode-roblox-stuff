...
import { setKeybindings, visibleWidth } from "@nyan-works/nyan-tui";
...
function userMessage(id, parentId, content) {
    return {
        type: "message",
        id,
        parentId,
...
function toolCallOnlyAssistant(id, parentId) {
    return {
        type: "message",
        id,
        parentId,
...
function modelChange(id, parentId) {
    return {
        type: "model_change",
        id,
        parentId,
...
describe("TreeSelectorComponent", () => {
    describe("initial selection with metadata entries", () => {
        test("focuses nearest visible ancestor when currentLeafId is a model_change with sibling branch", () => {
            ...
            const selector = new TreeSelectorComponent(tree, "model-1", // currentLeafId is the model_change entry
            24, () => { }, () => { });
    ...
    describe("branch navigation and folding with ctrl+arrow keys", () => {
        ...
        test("ctrl+right unfolds a folded node, then does segment jump when unfolded", () => {
            ...
            const selector = new TreeSelectorComponent(tree, "asst-4a", 24, () => { }, () => { });
            const list = selector.getTreeList();
...
//# sourceMappingURL=tree-selector.test.js.map