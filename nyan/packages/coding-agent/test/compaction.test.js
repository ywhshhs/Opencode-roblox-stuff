...
import { calculateContextTokens, compact, DEFAULT_COMPACTION_SETTINGS, estimateContextTokens, findCutPoint, getLastAssistantUsage, prepareCompaction, shouldCompact, } from "../src/core/compaction/index.js";
import { buildSessionContext, migrateSessionEntries, parseSessionEntries, } from "../src/core/session-manager.js";
// ============================================================================
// Test fixtures
// ============================================================================
...
function createAssistantMessage(text, usage) {
    return {
        role: "assistant",
...
function createCompactionEntry(summary, firstKeptEntryId) {
    const id = `test-id-${entryCounter++}`;
    const entry = {
        type: "compaction",
        id,
...
function createModelChangeEntry(provider, modelId) {
    const id = `test-id-${entryCounter++}`;
    const entry = {
        type: "model_change",
        id,
...
function createThinkingLevelEntry(thinkingLevel) {
    const id = `test-id-${entryCounter++}`;
    const entry = {
...
describe("getLastAssistantUsage", () => {
    ...
    it("should return undefined if no assistant messages", () => {
        const entries = [createMessageEntry(createUserMessage("Hello"))];
...
describe("findCutPoint", () => {
    it("should find cut point based on actual token differences", () => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            entries.push(createMessageEntry(createAssistantMessage(`Assistant ${i}`, createMockUsage(0, 100, (i + 1) * 1000, 0))));
        }
        ...
        const role = entries[result.firstKeptEntryIndex].message.role;
        expect(role === "user" || role === "assistant").toBe(true);
    });
    it("should return startIndex if no valid cut points in range", () => {
        const entries = [createMessageEntry(createAssistantMessage("a"))];
...
describe("buildSessionContext", () => {
    ...
    it("should handle single compaction", () => {
        ...
        const entries = [u1, a1, u2, a2, compaction, u3, a3];
        const loaded = buildSessionContext(entries);
        // summary + kept (u2, a2) + after (u3, a3) = 5
        expect(loaded.messages.length).toBe(5);
        expect(loaded.messages[0].role).toBe("compactionSummary");
        expect(loaded.messages[0].summary).toContain("Summary of 1,a,2,b");
    });
    it("should handle multiple compactions (only latest matters)", () => {
        ...
        const entries = [u1, a1, compact1, u2, b, u3, c, compact2, u4, d];
        const loaded = buildSessionContext(entries);
        // summary + kept from u3 (u3, c) + after (u4, d) = 5
        expect(loaded.messages.length).toBe(5);
        expect(loaded.messages[0].summary).toContain("Second summary");
    });
    it("should keep all messages when firstKeptEntryId is first entry", () => {
        ...
        const u2 = createMessageEntry(createUserMessage("2"));
        const b = createMessageEntry(createAssistantMessage("b"));
        const entries = [u1, a1, compact1, u2, b];
        const loaded = buildSessionContext(entries);
...
describe.skipIf(!process.env.ANTHROPIC_OAUTH_TOKEN)("LLM summarization", () => {
    ...
    it("should produce valid session after compaction", async () => {
        ...
        expect(reloaded.messages[0].summary).toContain(compactionResult.summary);
