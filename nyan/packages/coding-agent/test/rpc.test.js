 ...
 * RPC mode tests.
 */
describe.skipIf(!process.env.ANTHROPIC_ANYAN_KEY && !process.env.ANTHROPIC_OAUTH_TOKEN)("RPC mode", () => {
    let client;
    let sessionDir;
    beforeEach(() => {
        ...
        client = new RpcClient({
            ...
            cwd: join(__dirname, ".."),
            env: { NYAN_CODING_AGENT_DIR: sessionDir },
            provider: "anthropic",
    ...
    test("should add bash output to context", async () => {
        ...
        const entries = sessionContent
            ...
            .split("\n")
            .map((line) => JSON.parse(line));
        const bashMessages = entries.filter((e) => e.type === "message" && e.message?.role === "bashExecution");
    ...
    test("should include bash output in LLM context", async () => {
        ...
        const events = await client.promptAndWait("What was the exact output of the echo command I just ran? Reply with just the value, nothing else.");
        // Find assistant's response
        const messageEndEvents = events.filter((e) => e.type === "message_end");
        const assistantMessage = messageEndEvents.find((e) => e.type === "message_end" && e.message?.role === "assistant");
