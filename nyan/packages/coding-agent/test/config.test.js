...
import { detectInstallMethod, getSelfUpdateCommand, getSelfUpdateUnavailableInstruction, getUpdateInstruction, } from "../src/config.js";
const execPathDescriptor = Object.getOwnPropertyDescriptor(process, "execPath");
const originalPath = process.env.PATH;
const originalPiPackageDir = process.env.NYAN_PACKAGE_DIR;
...
afterEach(() => {
    ...
    if (originalPiPackageDir === undefined) {
        delete process.env.NYAN_PACKAGE_DIR;
    }
    else {
        process.env.NYAN_PACKAGE_DIR = originalPiPackageDir;
    }
...
function createNpmPrefixInstall(template = "pi-prefix-") {
    ...
    tempDir = prefix;
    process.env.NYAN_PACKAGE_DIR = packageDir;
...
function createPnpmGlobalInstall() {
    ...
    process.env.NYAN_PACKAGE_DIR = packageDir;
    setExecPath(join(root, ".pnpm", "@mariozechner+pi-coding-agent@0.0.0", "node_modules", "@mariozechner", "pi-coding-agent", "dist", "cli.js"));
...
function createFakePnpmScript(root) {
...
function createFakeYarnScript(globalDir) {
    if (process.platform === "win32") {
...
function createFakeBunScript(bunBin) {
    if (process.platform === "win32") {
...
describe("detectInstallMethod", () => {
    test("detects pnpm from Windows .pnpm install paths", () => {
        setExecPath("C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@earendil-works+pi-coding-agent@0.67.68\\node_modules\\@earendil-works\\pi-coding-agent\\dist\\cli.js");
        expect(detectInstallMethod()).toBe("pnpm");
        expect(getUpdateInstruction("@nyan-works/nyan-coding-agent")).toBe("Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @nyan-works/nyan-coding-agent");
    });
    test("does not self-update unknown wrapper installs", () => {
        setExecPath("/usr/local/bin/node");
        expect(detectInstallMethod()).toBe("unknown");
        expect(getSelfUpdateCommand("@nyan-works/nyan-coding-agent")).toBeUndefined();
        expect(getUpdateInstruction("@nyan-works/nyan-coding-agent")).toBe("Update @nyan-works/nyan-coding-agent using the package manager, wrapper, or source checkout that provides this installation.");
    });
    test("self-updates npm installs from custom prefixes", () => {
        const { prefix } = createNpmPrefixInstall();
        const command = getSelfUpdateCommand("@nyan-works/nyan-coding-agent");
        ...
        expect(command).toEqual({
            ...
            args: [
                ...
                "--ignore-scripts",
                "--min-release-age=0",
                "@nyan-works/nyan-coding-agent",
            ],
            display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @nyan-works/nyan-coding-agent`,
        });
    });
    test("self-updates exact npm versions without uninstalling the current package", () => {
        const { prefix } = createNpmPrefixInstall();
        const command = getSelfUpdateCommand("@nyan-works/nyan-coding-agent", undefined, {
            packageName: "@nyan-works/nyan-coding-agent",
            installSpec: "@nyan-works/nyan-coding-agent@1.2.3",
        });
        expect(command).toEqual({
            command: "npm",
            args: [
                ...
                "--ignore-scripts",
                "--min-release-age=0",
                "@nyan-works/nyan-coding-agent@1.2.3",
            ],
            display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @nyan-works/nyan-coding-agent@1.2.3`,
        });
    });
    ...
    test("self-update respects configured npmCommand", () => {
        const { prefix } = createNpmPrefixInstall();
        const command = getSelfUpdateCommand("@nyan-works/nyan-coding-agent", ["npm", "--prefix", prefix]);
        expect(command).toEqual({
            command: "npm",
            args: [
                "--prefix",
    ...
    test("quotes npm self-update display paths", () => {
        ...
        const command = getSelfUpdateCommand("@nyan-works/nyan-coding-agent");
        expect(command?.display).toBe(`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @nyan-works/nyan-coding-agent`);
    });
    test("does not infer Windows npm custom prefixes from package paths", () => {
        ...
        expect(detectInstallMethod()).toBe("npm");
        expect(getUpdateInstruction("@nyan-works/nyan-coding-agent")).toBe("Run: npm install -g --ignore-scripts --min-release-age=0 @nyan-works/nyan-coding-agent");
    });
    test("self-updates bun global installs from bun pm bin", () => {
        ...
        expect(command).toEqual({
            command: "bun",
            args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@nyan-works/nyan-coding-agent"],
            display: "bun install -g --ignore-scripts --minimum-release-age=0 @nyan-works/nyan-coding-agent",
        });
    });
    test("self-updates renamed pnpm global installs by removing the old package first", () => {
        ...
        expect(command).toEqual({
            ...
            display: "pnpm remove -g @mariozechner/pi-coding-agent && pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/pi",
            steps: [
                {
                    command: "pnpm",
    ...
    test("self-updates pnpm v11 global installs resolved through the store", () => {
        ...
        const packageName = "@nyan-works/nyan-coding-agent";
        const globalPackageDir = join(root, "11e9a", "node_modules", "@earendil-works", "pi-coding-agent");
        const storePackageDir = join(temp, "Library", "pnpm", "store", "v11", "links", "@earendil-works", "pi-coding-agent", "0.75.0", "hash", "node_modules", "@earendil-works", "pi-coding-agent");
        ...
        tempDir = temp;
        process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
        process.env.NYAN_PACKAGE_DIR = storePackageDir;
    ...
    test("self-updates renamed bun global installs by removing the old package first", () => {
        ...
        expect(command).toEqual({
            ...
            display: "bun uninstall -g @mariozechner/pi-coding-agent && bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/pi",
            steps: [
                {
                    command: "bun",
    ...
    test("does not self-update when npm install path is not writable", () => {
        ...
        expect(getSelfUpdateCommand("@nyan-works/nyan-coding-agent")).toBeUndefined();
        expect(getSelfUpdateUnavailableInstruction("@nyan-works/nyan-coding-agent")).toContain("the install path is not writable");
    });
});
