...
import { createEditTool, createFindTool, createGrepTool, createLsTool, createReadTool, createWriteTool, } from "../src/index.js";
...
function createTinyBmp1x1Red24bpp() {
...
describe("Coding Agent Tools", () => {
    ...
    describe("read tool", () => {
        ...
        it("should show error when offset is beyond file length", async () => {
            ...
            await expect(readTool.execute("test-call-8", { path: testFile, offset: 100 })).rejects.toThrow(/Offset 100 is beyond end of file \(3 lines total\)/);
        });
    ...
    describe("edit tool", () => {
        ...
        it("should fail if text not found", async () => {
            ...
            const originalContent = "Hello, world!";
            writeFileSync(testFile, originalContent);
            await expect(editTool.execute("test-call-6", {
                path: testFile,
        ...
        it("should include ENOENT when the edit target does not exist", async () => {
            ...
            await expect(editTool.execute("test-call-6b", {
                path: missingFile,
        ...
        it("should fail if text appears multiple times", async () => {
            ...
            writeFileSync(testFile, originalContent);
            await expect(editTool.execute("test-call-7", {
                path: testFile,
        ...
        it("should fail when edits is empty", async () => {
            ...
            await expect(editTool.execute("test-call-11", {
                path: testFile,
                edits: [],
        ...
        it("should fail when multi-edit regions overlap", async () => {
            ...
            await expect(editTool.execute("test-call-12", {
                path: testFile,
                edits: [
                    { oldText: "one\ntwo\n", newText: "ONE\nTWO\n" },
                    { oldText: "two\nthree\n", newText: "TWO\nTHREE\n" },
        ...
        it("should not partially apply edits when one edit fails", async () => {
            ...
            writeFileSync(testFile, originalContent);
            await expect(editTool.execute("test-call-13", {
                path: testFile,
                edits: [
                    { oldText: "alpha\n", newText: "ALPHA\n" },
                    { oldText: "missing\n", newText: "MISSING\n" },
        ...
        it("should include EACCES for read-only files", async () => {
            ...
            await expect(editTool.execute("test-call-14", {
                path: testFile,
                edits: [{ oldText: "hello", newText: "world" }],
        ...
        it("should include the original error message for unknown edit access errors", async () => {
            ...
            await expect(genericFailureTool.execute("test-call-16", {
                path: "broken.txt",
                edits: [{ oldText: "hello", newText: "world" }],
    ...
    describe("bash tool", () => {
        ...
        it("should throw error when cwd does not exist", async () => {
            ...
            await expect(bashToolWithBadCwd.execute("test-call-11", { command: "echo test" })).rejects.toThrow(/Working directory does not exist/);
        });
        ...
        it("should pass shellPath through to shell resolution", async () => {
            ...
            await expect(ops.exec("echo test", testDir, {
                onData: () => { },
        ...
        it("should coalesce streaming updates for chatty output", async () => {
            ...
            const updates = [];
            const bash = createBashTool(testDir, { operations });
            const result = await bash.execute("test-call-chatty-updates", { command: "chatty" }, undefined, (update) => updates.push(update));
        ...
        it("should preserve executeBash sanitization when using local bash operations", async () => {
            const result = await executeBashWithOperations("printf '\\033[31mred\\033[0m\\r\\n'", process.cwd(), createLocalBashOperations());
            expect(result.exitCode).toBe(0);
    ...
    describe("find tool", () => {
        ...
        it("should surface fd glob parse errors", async () => {
            await expect(findTool.execute("test-call-15", {
                pattern: "[",
                path: testDir,
...
describe("edit tool CRLF handling", () => {
    ...
    it("should detect duplicates across CRLF/LF variants", async () => {
        ...
        writeFileSync(testFile, "hello\r\nworld\r\n---\r\nhello\nworld\n");
        await expect(editTool.execute("test-crlf-dup", {
            path: testFile,
            edits: [{ oldText: "hello\nworld\n", newText: "replaced\n" }],
        })).rejects.toThrow(/Found 2 occurrences/);
    });
