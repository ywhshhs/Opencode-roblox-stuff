...
describe("session selector rename", () => {
    ...
    it("shows rename hint in interactive /resume picker configuration", async () => {
        ...
        const selector = new SessionSelectorComponent(async () => sessions, async () => [], () => { }, () => { }, () => { }, () => { }, { showRenameHint: true, keybindings });
        await flushPromises();
    ...
    it("does not show rename hint in --resume picker configuration", async () => {
        const sessions = [makeSession({ id: "a" })];
        const keybindings = new KeybindingsManager();
        const selector = new SessionSelectorComponent(async () => sessions, async () => [], () => { }, () => { }, () => { }, () => { }, { showRenameHint: false, keybindings });
        await flushPromises();
        const output = selector.render(120).join("\n");
    ...
    it("enters rename mode on Ctrl+R and submits with Enter", async () => {
        ...
        const keybindings = new KeybindingsManager();
        const selector = new SessionSelectorComponent(async () => sessions, async () => [], () => { }, () => { }, () => { }, () => { }, { renameSession, showRenameHint: true, keybindings });
        await flushPromises();
...
//# sourceMappingURL=session-selector-rename.test.js.map