...
describe("extensions discovery", () => {
	...
	`;
    const extensionCodeWithTool = (toolName) => `
    ...
    it("keeps package.json pi extension entries with leading tilde package-relative", async () => {
        ...
        const result = await discoverAndLoadExtensions([], tempDir, tempDir);
        expect(result.errors).toHaveLength(0);
        expect(result.extensions.map((extension) => extension.path).sort()).toEqual([directExtensionPath, slashExtensionPath].sort());
    });
...
//# sourceMappingURL=extensions-discovery.test.js.map