...
import { getModel, Type, } from "@nyan-works/nyan-ai/compat";
import { getOpenAICodexWebSocketDebugStats, streamSimple as streamSimpleOpenAICodexResponses, } from "../../ai/src/api/openai-codex-responses.js";
...
function buildPrompt(turn) {
    ...
    lines.push("");
    for (let i = 1; i <= 180; i++) {
        lines.push(`Turn ${turn} synthetic record ${String(i).padStart(3, "0")}: alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi omicron pi rho sigma tau upsilon phi chi psi omega.`);
    }
    lines.push("");
...
function createMinimalResourceLoader(systemPrompt) {
    return {
...
function getWebSocketStatsSnapshot(sessionId) {
...
function diffWebSocketStats(after, before) {
    return {
...
function formatWebSocketStats(label, stats) {
    if (stats.requests === 0)
        return `${label} websocket none`;
    return [
        `${label} websocket`,
...
function deterministicProbeTool() {
    return {
        name: "deterministic_probe",
        label: "Deterministic Probe",
        description: "Mandatory cache-benchmark tool. Call it exactly once when the user asks for a cache benchmark turn, then use its result to produce the final one-line answer.",
        promptSnippet: "deterministic_probe(turn, marker): mandatory for cache benchmark turns. Call exactly once before the final answer.",
        promptGuidelines: [
...
async function main() {
    ...
    const streamSimpleOpenAICodexResponsesForRegistry = (registryModel, context, options) => streamSimpleOpenAICodexResponses(registryModel, context, options);
    ...
    for (let turn = 1; turn <= args.turns; turn++) {
        ...
        const assistantMessages = newMessages.filter((message) => Boolean(message && typeof message === "object" && message.role === "assistant"));
        const toolResults = newMessages.filter((message) => Boolean(message && typeof message === "object" && message.role === "toolResult"));
        if (assistantMessages.length < 2 || toolResults.length < 1) {
            throw new Error(`Turn ${turn} did not execute the expected tool loop. assistants=${assistantMessages.length} toolResults=${toolResults.length}`);
        }
        let turnInput = 0;
        let turnOutput = 0;
        let turnCacheRead = 0;
        ...
        for (let i = 0; i < assistantMessages.length; i++) {
            ...
            const monotonic = previousCacheRead === null ? "n/a" : assistant.usage.cacheRead >= previousCacheRead ? "yes" : "NO";
            console.log([
            ...
            if (assistant.stopReason === "error" || assistant.stopReason === "aborted") {
                throw new Error(`Turn ${turn}.${i + 1} ended with stopReason=${assistant.stopReason}: ${assistant.errorMessage || "unknown error"}`);
            }
    ...
    const violations = records
        ...
        if (record.usage.cacheRead >= previous.usage.cacheRead)
            return null;
        return {
            turn: record.turn,
    ...
    const requestedWebsocket = args.transport === "websocket" || args.transport === "websocket-cached" || args.transport === "auto";
...
//# sourceMappingURL=sdk-codex-cache-probe-tool-loop.js.map