...
describe("Input Event", () => {
    ...
    async function createRunner(...extensions) {
    ...
    it("transforms text and preserves images when omitted", async () => {
        const r = await createRunner(`export default p => p.on("input", async e => ({ action: "transform", text: "T:" + e.text }));`);
        const imgs = [{ type: "image", data: "orig", mimeType: "image/png" }];
    ...
    it("transforms and replaces images when provided", async () => {
        const r = await createRunner(`export default p => p.on("input", async () => ({ action: "transform", text: "X", images: [{ type: "image", data: "new", mimeType: "image/jpeg" }] }));`);
    ...
    it("chains transforms across multiple handlers", async () => {
        const r = await createRunner(`export default p => p.on("input", async e => ({ action: "transform", text: e.text + "[1]" }));`, `export default p => p.on("input", async e => ({ action: "transform", text: e.text + "[2]" }));`);
    ...
    it("short-circuits on handled and skips subsequent handlers", async () => {
        ...
        const r = await createRunner(`export default p => p.on("input", async () => ({ action: "handled" }));`, `export default p => p.on("input", async () => { globalThis.testVar = true; });`);
        expect(await r.emitInput("X", undefined, "interactive")).toEqual({ action: "handled" });
        expect(globalThis.testVar).toBe(false);
    });
    it("passes source correctly for all source types", async () => {
        const r = await createRunner(`export default p => p.on("input", async e => { globalThis.testVar = e.source; return { action: "continue" }; });`);
        for (const source of ["interactive", "rpc", "extension"]) {
            await r.emitInput("x", undefined, source);
            expect(globalThis.testVar).toBe(source);
        }
    });
    it("passes streamingBehavior correctly", async () => {
        const r = await createRunner(`export default p => p.on("input", async e => { globalThis.testVar = e.streamingBehavior; return { action: "continue" }; });`);
        await r.emitInput("x", undefined, "interactive", "steer");
        expect(globalThis.testVar).toBe("steer");
        await r.emitInput("x", undefined, "interactive", "followUp");
        expect(globalThis.testVar).toBe("followUp");
        await r.emitInput("x", undefined, "interactive");
        expect(globalThis.testVar).toBeUndefined();
    });
...
//# sourceMappingURL=extensions-input-event.test.js.map