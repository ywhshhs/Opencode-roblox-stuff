...
---
Project skill`);
            const baseTheme = JSON.parse(readFileSync(join(process.cwd(), "src", "modes", "interactive", "theme", "dark.json"), "utf-8"));
        ...
        it("should load symlinked user and project extensions once", async () => {
            ...
            writeFileSync(join(sharedExtDir, "shared.ts"), `export default function(pi) {
	pi.registerCommand("shared", {
        ...
        it("should load user extensions before trust and reuse them after trust resolves", async () => {
            ...
            writeFileSync(join(userExtDir, "user.ts"), `globalThis[${JSON.stringify(loadCountKey)}] = (globalThis[${JSON.stringify(loadCountKey)}] ?? 0) + 1;
export default function(pi) {
	...
	pi.registerCommand("user-trust", {
		...
		handler: async () => {},
            ...
            writeFileSync(join(projectExtDir, "project.ts"), `export default function(pi) {
	...
	pi.registerCommand("project-only", {
		...
		handler: async () => {},
            ...
            writeFileSync(join(userExtDir, "user.ts"), `export default function(pi) {
	pi.registerCommand("deploy", {
	...
	pi.registerCommand("user-only", {
		...
		handler: async () => {},
            ...
            const runner = new ExtensionRunner(extensionsResult.extensions, extensionsResult.runtime, cwd, sessionManager, modelRegistry);
...
Project skill content`);
            ...
            const themeData = JSON.parse(readFileSync(join(process.cwd(), "src", "modes", "interactive", "theme", "dark.json"), "utf-8"));
...
Content`);
        ...
        it("should still load additional skill paths when noSkills is true", async () => {
            ...
            writeFileSync(join(customSkillDir, "custom.md"), `---
name: custom
...
Content`);
    ...
    describe("extension conflict detection", () => {
        it("should detect tool conflicts between extensions", async () => {
            ...
            writeFileSync(join(ext1Dir, "index.ts"), `
...
export default function(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    execute: async () => ({ result: "1" }),
            ...
            writeFileSync(join(ext2Dir, "index.ts"), `
import type { ExtensionAPI } from "@nyan-works/nyan-coding-agent";
import { Type } from "typebox";
export default function(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    execute: async () => ({ result: "2" }),
        ...
        it("should prefer explicit CLI extensions over discovered extensions when commands and tools conflict", async () => {
            ...
            writeFileSync(join(globalExtDir, "global.ts"), `
import type { ExtensionAPI } from "@nyan-works/nyan-coding-agent";
import { Type } from "typebox";
export default function(pi: ExtensionAPI) {
  pi.registerTool({
...
//# sourceMappingURL=resource-loader.test.js.map