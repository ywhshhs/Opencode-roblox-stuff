...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@nyan-works/nyan-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @nyan-works/nyan-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getUpdateInstruction("@nyan-works/nyan-coding-agent")).toBe(
			"Update @nyan-works/nyan-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @nyan-works/nyan-coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@nyan-works/nyan-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @nyan-works/nyan-coding-agent",
		);
	});
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@nyan-works/nyan-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@nyan-works/nyan-coding-agent")).toContain(
