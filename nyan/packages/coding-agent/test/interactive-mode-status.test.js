...
import { CombinedAutocompleteProvider } from "@nyan-works/nyan-tui";
import { beforeAll, describe, expect, test, vi } from "vitest";
import { Container, TUI } from "../../tui/src/tui.js";
import { VirtualTerminal } from "../../tui/test/virtual-terminal.js";
import { InteractiveMode } from "../src/modes/interactive/interactive-mode.js";
import { initTheme } from "../src/modes/interactive/theme/theme.js";
function renderLastLine(container, width = 120) {
    const last = container.children[container.children.length - 1];
    if (!last)
        return "";
    return last.render(width).join("\n");
}
function renderAll(container, width = 120) {
    return container.children.flatMap((child) => child.render(width)).join("\n");
}
class TestFocusableComponent {
    constructor(label) {
        this.focused = false;
        this.inputs = [];
        this.text = "";
...
async function flushTui(tui, terminal) {
    tui.requestRender(true);
    await Promise.resolve();
    await terminal.waitForRender();
}
function normalizeRenderedOutput(container, width = 220) {
...
describe("InteractiveMode.showStatus", () => {
    ...
    test("coalesces immediately-sequential status messages", () => {
        ...
        InteractiveMode.prototype.showStatus.call(fakeThis, "STATUS_ONE");
        expect(fakeThis.chatContainer.children).toHaveLength(2);
        expect(renderLastLine(fakeThis.chatContainer)).toContain("STATUS_ONE");
        InteractiveMode.prototype.showStatus.call(fakeThis, "STATUS_TWO");
...
describe("InteractiveMode.createExtensionUIContext setTheme", () => {
    test("persists theme changes to settings manager", () => {
        ...
        const uiContext = InteractiveMode.prototype.createExtensionUIContext.call(fakeThis);
...
describe("InteractiveMode.showExtensionCustom", () => {
    ...
    test("overlay custom UI reclaims input after non-overlay custom UI closes", async () => {
        ...
        const showExtensionCustom = (factory, options) => InteractiveMode.prototype.showExtensionCustom.call(fakeThis, factory, options);
        ...
        ui.start();
        try {
            const overlayPromise = showExtensionCustom((_tui, _theme, _keybindings, done) => {
                closeOverlay = done;
                return overlay;
            }, { overlay: true });
            await flushTui(ui, terminal);
            expect(overlay.focused).toBe(true);
            const replacementPromise = showExtensionCustom((_tui, _theme, _keybindings, done) => {
...
describe("InteractiveMode.setupAutocompleteProvider", () => {
    test("stacks wrapper factories over a fresh base provider", () => {
        ...
        InteractiveMode.prototype.setupAutocompleteProvider.call(fakeThis);
        ...
        const provider = defaultEditor.setAutocompleteProvider.mock.calls[0]?.[0];
    ...
    test("merges triggerCharacters from wrapper factories", () => {
        const defaultEditor = { setAutocompleteProvider: vi.fn() };
        const customEditor = { setAutocompleteProvider: vi.fn() };
        const passThrough = (triggerCharacters) => (current) => ({
            triggerCharacters,
            getSuggestions: (lines, cursorLine, cursorCol, options) => current.getSuggestions(lines, cursorLine, cursorCol, options),
...
describe("InteractiveMode.createBaseAutocompleteProvider", () => {
    test("matches model command arguments across provider/model order", async () => {
        const createBaseAutocompleteProvider = InteractiveMode.prototype.createBaseAutocompleteProvider;
        const models = [
...
describe("InteractiveMode.showLoadedResources", () => {
    ...
    function createShowLoadedResourcesThis(options) {
        const fakeThis = {
            ...
            formatDisplayPath: (p) => InteractiveMode.prototype.formatDisplayPath.call(fakeThis, p),
            formatExtensionDisplayPath: (p) => InteractiveMode.prototype.formatExtensionDisplayPath.call(fakeThis, p),
            formatContextPath: (p) => InteractiveMode.prototype.formatContextPath.call(fakeThis, p),
            getStartupExpansionState: () => InteractiveMode.prototype.getStartupExpansionState.call(fakeThis),
            buildScopeGroups: () => [],
            formatScopeGroups: () => "resource-list",
            isPackageSource: (sourceInfo) => InteractiveMode.prototype.isPackageSource.call(fakeThis, sourceInfo),
            getShortPath: (p, sourceInfo) => InteractiveMode.prototype.getShortPath.call(fakeThis, p, sourceInfo),
            getCompactPathLabel: (p, sourceInfo) => InteractiveMode.prototype.getCompactPathLabel.call(fakeThis, p, sourceInfo),
            getCompactPackageSourceLabel: (sourceInfo) => InteractiveMode.prototype.getCompactPackageSourceLabel.call(fakeThis, sourceInfo),
            getCompactExtensionLabel: (p, sourceInfo) => InteractiveMode.prototype.getCompactExtensionLabel.call(fakeThis, p, sourceInfo),
            getCompactDisplayPathSegments: (p) => InteractiveMode.prototype.getCompactDisplayPathSegments.call(fakeThis, p),
            getCompactNonPackageExtensionLabel: (p, index, allPaths) => InteractiveMode.prototype.getCompactNonPackageExtensionLabel.call(fakeThis, p, index, allPaths),
            getCompactExtensionLabels: (extensions) => InteractiveMode.prototype.getCompactExtensionLabels.call(fakeThis, extensions),
            formatDiagnostics: () => "diagnostics",
            getBuiltInCommandConflictDiagnostics: () => [],
        };
        if (options.useRealScopeGroups) {
            fakeThis.getScopeGroup = (sourceInfo) => InteractiveMode.prototype.getScopeGroup.call(fakeThis, sourceInfo);
            fakeThis.buildScopeGroups = (items) => InteractiveMode.prototype.buildScopeGroups.call(fakeThis, items);
            fakeThis.formatScopeGroups = (groups, formatOptions) => InteractiveMode.prototype.formatScopeGroups.call(fakeThis, groups, formatOptions);
        }
        return fakeThis;
    }
    function createSourceInfo(filePath, options) {
        return {
            path: filePath,
    ...
    function createExtensionFixtures() {
        return [
            {
                path: "/tmp/project/.nyan/extensions/answer.ts",
                sourceInfo: createSourceInfo("/tmp/project/.nyan/extensions/answer.ts", {
                    source: "local",
                    scope: "project",
                    origin: "top-level",
                    baseDir: "/tmp/project/.nyan/extensions",
                }),
            },
            {
                path: "/tmp/project/.nyan/extensions/local-index/index.ts",
                sourceInfo: createSourceInfo("/tmp/project/.nyan/extensions/local-index/index.ts", {
                    source: "local",
                    scope: "project",
                    origin: "top-level",
                    baseDir: "/tmp/project/.nyan/extensions",
            ...
            {
                path: "/tmp/project/.nyan/npm/node_modules/pi-markdown-preview/extensions/index.ts",
                sourceInfo: createSourceInfo("/tmp/project/.nyan/npm/node_modules/pi-markdown-preview/extensions/index.ts", {
                    source: "npm:pi-markdown-preview",
                    scope: "project",
                    origin: "package",
                    baseDir: "/tmp/project/.nyan/npm/node_modules/pi-markdown-preview",
                }),
            },
            {
                path: "/tmp/project/.nyan/npm/node_modules/@scope/pi-scoped/extensions/index.ts",
                sourceInfo: createSourceInfo("/tmp/project/.nyan/npm/node_modules/@scope/pi-scoped/extensions/index.ts", {
                    source: "npm:@scope/pi-scoped",
                    scope: "project",
                    origin: "package",
                    baseDir: "/tmp/project/.nyan/npm/node_modules/@scope/pi-scoped",
                }),
            },
            {
                path: "/tmp/project/.nyan/git/github.com/HazAT/pi-interactive-subagents/extensions/index.ts",
                sourceInfo: createSourceInfo("/tmp/project/.nyan/git/github.com/HazAT/pi-interactive-subagents/extensions/index.ts", {
                    source: "git:github.com/HazAT/pi-interactive-subagents",
                    scope: "project",
                    origin: "package",
                    baseDir: "/tmp/project/.nyan/git/github.com/HazAT/pi-interactive-subagents",
                }),
            },
            {
                path: "/tmp/project/.nyan/git/github.com/HazAT/pi-interactive-subagents/extensions/subagents/index.ts",
                sourceInfo: createSourceInfo("/tmp/project/.nyan/git/github.com/HazAT/pi-interactive-subagents/extensions/subagents/index.ts", {
                    source: "git:github.com/HazAT/pi-interactive-subagents",
                    scope: "project",
                    origin: "package",
                    baseDir: "/tmp/project/.nyan/git/github.com/HazAT/pi-interactive-subagents",
...
"[Extensions]
  project
    /tmp/project/.nyan/extensions/answer.ts
    /tmp/project/.nyan/extensions/local-index
  ...
  path
    ...
    test("shows context paths relative to cwd while preserving full external paths", () => {
        ...
        expect(output).toContain("~/.nyan/agent/AGENTS.md, AGENTS.md");
    ...
    test("shows full context paths when expanded", () => {
        ...
        const output = renderAll(fakeThis.loadedResourcesContainer).replace(/\\/g, "/");
        expect(output).toContain("[Context]");
        expect(output).toContain("~/.nyan/agent/AGENTS.md");
        expect(output).toContain("~/Development/pi-mono/AGENTS.md");
        expect(output).not.toContain("~/.nyan/agent/AGENTS.md, AGENTS.md");
    });
...
//# sourceMappingURL=interactive-mode-status.test.js.map