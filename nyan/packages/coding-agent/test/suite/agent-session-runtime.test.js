...
import { createAgentSessionFromServices, createAgentSessionRuntime, createAgentSessionServices, } from "../../src/core/agent-session-runtime.js";
...
describe("AgentSessionRuntime characterization", () => {
    ...
    async function createRuntimeForTest(extensionFactory, options) {
        const tempDir = options?.cwd ?? join(tmpdir(), `pi-runtime-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    it("duplicates the current active branch when forking at the current position", async () => {
        ...
        expect(runtime.session.messages.map((message) => ({
            role: message.role,
            text: message.role === "user"
                ? typeof message.content === "string"
                    ? message.content
    ...
    it("updates the runtime session cwd on cross-cwd session replacement", async () => {
        ...
        const createOtherRuntime = async ({ cwd, sessionManager, sessionStartEvent, }) => {
            const services = await createAgentSessionServices({
                ...otherRuntimeOptions,
                cwd,
            });
...
//# sourceMappingURL=agent-session-runtime.test.js.map