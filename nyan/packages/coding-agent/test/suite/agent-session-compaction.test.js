import { createAssistantMessageEventStream, fauxAssistantMessage, } from "@nyan-works/nyan-ai";
import { afterEach, describe, expect, it, vi } from "vitest";
import { estimateTokens } from "../../src/core/compaction/index.js";
import { createHarness } from "./harness.js";
function createUsage(totalTokens) {
    return {
        input: totalTokens,
        output: 0,
        cacheRead: 0,
...
function createAssistant(harness, options) {
...
function useSummaryStreamFn(harness, summary) {
    let callCount = 0;
...
function seedCompactableSession(harness) {
...
describe("AgentSession compaction characterization", () => {
    ...
    it("does not retry overflow recovery more than once", async () => {
        ...
        expect(compactionErrors).toContain("Context overflow recovery failed after one compact-and-retry attempt. Try reducing context or switching to a larger-context model.");
    });
    ...
    it("ignores stale pre-compaction assistant usage on pre-prompt checks", async () => {
        ...
        harness.sessionManager.appendCompaction("summary", firstKeptEntryId, staleAssistant.usage.totalTokens, undefined, false);
        harness.sessionManager.appendMessage({
            role: "user",
    ...
    it("does not trigger threshold compaction when only kept pre-compaction usage exists", async () => {
        ...
        const firstKeptEntryId = harness.sessionManager.getEntries()[0].id;
        harness.sessionManager.appendCompaction("summary", firstKeptEntryId, keptAssistant.usage.totalTokens, undefined, false);
        const errorAssistant = createAssistant(harness, {
            stopReason: "error",
            errorMessage: "529 overloaded",
            timestamp: Date.now(),
    ...
    it("does not trigger threshold compaction below the threshold or when disabled", async () => {
        ...
        const disabledInternals = disabledHarness.session;
        ...
        await belowThresholdInternals._checkCompaction(createAssistant(belowThresholdHarness, { stopReason: "stop", totalTokens: 1_000, timestamp: Date.now() }));
        await disabledInternals._checkCompaction(createAssistant(disabledHarness, { stopReason: "stop", totalTokens: 1_000_000, timestamp: Date.now() }));
        expect(belowThresholdSpy).not.toHaveBeenCalled();
        expect(disabledSpy).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=agent-session-compaction.test.js.map