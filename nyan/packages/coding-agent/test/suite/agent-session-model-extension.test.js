...
describe("AgentSession model and extension characterization", () => {
    ...
    it("setModel saves the model and emits model_select", async () => {
        ...
        expect(harness.sessionManager
            ...
            .map((entry) => `${entry.provider}/${entry.modelId}`)).toEqual([`${nextModel.provider}/${nextModel.id}`]);
    });
    ...
    it("throws when setModel is called without configured auth", async () => {
        ...
        harnesses.push(harness);
        await expect(harness.session.setModel(harness.getModel("faux-2"))).rejects.toThrow(`No API key for ${harness.getModel().provider}/faux-2`);
    });
    it("allows extension tool_call handlers to block tool execution", async () => {
        ...
        harness.setResponses([
            ...
            (context) => {
                ...
                const errorText = toolResult?.role === "toolResult"
                    ? toolResult.content
    ...
    it("allows extension tool_result handlers to modify tool results", async () => {
        ...
        harness.setResponses([
            ...
            (context) => {
                const toolResult = context.messages.find((message) => message.role === "toolResult");
                const text = toolResult?.role === "toolResult"
                    ? toolResult.content
                        .filter((part) => part.type === "text")
                        .map((part) => part.text)
                        .join("\n")
        ...
        expect(harness.session.messages.find((message) => message.role === "toolResult" && message.details?.patched === true)).toBeDefined();
    });
    ...
    it("allows before_agent_start handlers to inject custom messages and modify the system prompt", async () => {
        ...
        harness.setResponses([
            (context) => {
                ...
                sawInjectedUserMessage = context.messages.some((message) => message.role === "user" &&
        ...
        expect(harness.session.messages.some((message) => message.role === "custom" && message.customType === "before-start")).toBe(true);
    });
...
//# sourceMappingURL=agent-session-model-extension.test.js.map