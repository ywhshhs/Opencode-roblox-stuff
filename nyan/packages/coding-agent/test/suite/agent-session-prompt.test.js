...
describe("AgentSession prompt characterization", () => {
    ...
    it("throws when prompted during streaming without a streamingBehavior", async () => {
        ...
        const promptPromise = harness.session.prompt("start");
        await sawToolStart;
        await expect(harness.session.prompt("second")).rejects.toThrow("Agent is already processing. Specify streamingBehavior ('steer' or 'followUp') to queue the message.");
        releaseToolExecution?.();
        await promptPromise;
    });
...
//# sourceMappingURL=agent-session-prompt.test.js.map