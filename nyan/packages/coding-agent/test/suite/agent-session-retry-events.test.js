import { fauxAssistantMessage, fauxThinking, fauxToolCall } from "@nyan-works/nyan-ai";
...
function normalizeEventOrder(events) {
    ...
    for (const event of events) {
        const label = event.type === "message_start" || event.type === "message_end"
...
describe("AgentSession retry and event characterization", () => {
    ...
    it("retries after a transient error and succeeds", async () => {
        ...
        harness.session.subscribe((event) => {
            ...
            if (event.type === "auto_retry_end")
                retryEvents.push(`end:${event.success}`);
        });
    ...
    it("emits streaming deltas for text, thinking, and tool calls in message_update events", async () => {
        ...
        harnesses.push(harness);
        harness.setResponses([
            fauxAssistantMessage([fauxThinking("plan"), { type: "text", text: "answer" }, fauxToolCall("echo", { text: "hello" })], {
                stopReason: "toolUse",
            }),
        ]);
    ...
    it("emits agent_end for aborted runs and persists the aborted assistant message", async () => {
        ...
        const sawMessageUpdate = new Promise((resolve) => {
            const unsubscribe = harness.session.subscribe((event) => {
...
//# sourceMappingURL=agent-session-retry-events.test.js.map