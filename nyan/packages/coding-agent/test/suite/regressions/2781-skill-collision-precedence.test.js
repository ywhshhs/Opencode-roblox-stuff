...
describe("issue #2781 skill collision precedence: user skills should override package skills", () => {
    ...
    function createPackageWithSkill(name, description) {
        const pkgDir = join(tempDir, `fake-package-${name}`);
        const skillDir = join(pkgDir, "skills", name);
        mkdirSync(skillDir, { recursive: true });
        writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: `fake-pkg-${name}`, version: "1.0.0", pi: { skills: [`skills/${name}`] } }, null, 2));
        writeFileSync(join(skillDir, "SKILL.md"), `---\nname: ${name}\ndescription: ${description}\n---\nPackage skill content`);
        return pkgDir;
    }
    function createUserSkill(name, description) {
    ...
    function createProjectSkill(name, description) {
    ...
    function createSettingsWithPackage(pkgDir, scope) {
...
//# sourceMappingURL=2781-skill-collision-precedence.test.js.map