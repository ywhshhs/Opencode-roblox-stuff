...
function pathEndsWith(actualPath, suffix) {
...
const isEnabled = (r, pathMatch, matchFn = "endsWith") => {
...
const isDisabled = (r, pathMatch, matchFn = "endsWith") => {
    const normalizedPath = normalizeForMatch(r.path);
    const normalizedMatch = normalizeForMatch(pathMatch);
    return matchFn === "endsWith"
        ? normalizedPath.endsWith(normalizedMatch) && !r.enabled
...
describe("DefaultPackageManager", () => {
    ...
    let settingsManager;
    let packageManager;
    let previousOfflineEnv;
    beforeEach(() => {
        previousOfflineEnv = process.env.NYAN_OFFLINE;
        delete process.env.NYAN_OFFLINE;
    ...
    afterEach(() => {
        if (previousOfflineEnv === undefined) {
            ...
            process.env.NYAN_OFFLINE = previousOfflineEnv;
        }
        vi.restoreAllMocks();
        vi.unstubAllGlobals();
...
Content`);
        ...
        it("should resolve symlinked user and project resources once", async () => {
            ...
            try {
                ...
                writeFileSync(join(sharedSkillsDir, "shared-skill", "SKILL.md"), `---
name: shared-skill
...
Content`);
    ...
    describe("auto-discovered skill metadata", () => {
        it("should use the agent dir as baseDir for user .nyan/agent skills", async () => {
    ...
    describe(".agents/skills auto-discovery", () => {
        ...
        it("should dedupe user skill entries when ~/.nyan/agent/skills is a symlink to ~/.agents/skills", async () => {
            const previousHome = process.env.HOME;
            process.env.HOME = tempDir;
            try {
    ...
    describe("resolveExtensionSources", () => {
        ...
        it("should handle directories with pi manifest", async () => {
            ...
            writeFileSync(join(pkgDir, "skills", "my-skill", "SKILL.md"), "---\nname: my-skill\ndescription: Test\n---\nContent");
            const result = await packageManager.resolveExtensionSources([pkgDir]);
            expect(result.extensions.some((r) => r.path === join(pkgDir, "src", "index.ts") && r.enabled)).toBe(true);
            // Skills with SKILL.md are returned as file paths
            expect(result.skills.some((r) => r.path === join(pkgDir, "skills", "my-skill", "SKILL.md") && r.enabled)).toBe(true);
        });
    ...
    describe("command spawning", () => {
        it("should preserve argv entries containing spaces", () => {
            const managerWithInternals = packageManager;
    ...
    describe("npmCommand", () => {
        it("should use npmCommand argv for npm installs", async () => {
            ...
            const runCommandSpy = vi.spyOn(packageManager, "runCommand").mockResolvedValue(undefined);
            await packageManager.install("npm:@scope/pkg");
            expect(runCommandSpy).toHaveBeenCalledWith("mise", [
                "exec",
                "node@20",
                "--",
                "npm",
        ...
        it("should use bun --cwd for npm package installs", async () => {
            ...
            const runCommandSpy = vi.spyOn(packageManager, "runCommand").mockResolvedValue(undefined);
            await packageManager.install("npm:@scope/pkg");
            expect(runCommandSpy).toHaveBeenCalledWith("mise", ["exec", "bun@1", "--", "bun", "install", "@scope/pkg", "--cwd", join(agentDir, "npm"), "--omit=peer"], undefined);
        });
        ...
        it("should update git package dependencies with --omit=dev", async () => {
            ...
            vi.spyOn(packageManager, "runCommandCapture").mockImplementation(async (...callArgs) => {
                const [_command, args] = callArgs;
        ...
        it("should use npmCommand argv for npm root lookup and invalidate cached root when npmCommand changes", () => {
            ...
            const runCommandSyncSpy = vi
                .spyOn(packageManager, "runCommandSync")
        ...
        it("should install user npm packages into the pi-managed npm root", async () => {
            ...
            vi.spyOn(packageManager, "runCommandSync").mockImplementation(() => {
            ...
            expect(first.extensions.some((r) => r.path === join(packagePath, "extensions", "index.ts") && r.enabled)).toBe(true);
            expect(second.extensions.some((r) => r.path === join(packagePath, "extensions", "index.ts") && r.enabled)).toBe(true);
        ...
        it("should load legacy pnpm global package paths from pnpm list output", async () => {
            ...
            writeFileSync(join(packagePath, "package.json"), JSON.stringify({ name: "pnpm-pkg", version: "1.0.0" }));
            writeFileSync(join(packagePath, "extensions", "index.ts"), "export default function() {};");
            vi.spyOn(packageManager, "runCommandSync").mockImplementation((...callArgs) => {
                const [command, args] = callArgs;
                if (command !== "pnpm") {
                    throw new Error(`unexpected command ${command}`);
                }
            ...
            const runCommandSpy = vi.spyOn(packageManager, "runCommand").mockResolvedValue(undefined);
            const result = await packageManager.resolve();
            expect(result.extensions.some((r) => r.path === join(packagePath, "extensions", "index.ts") && r.enabled)).toBe(true);
        ...
        it("should ignore malformed legacy pnpm global package lists", () => {
            ...
            vi.spyOn(packageManager, "runCommandSync").mockReturnValue("not json");
    ...
    describe("source parsing", () => {
        ...
        it("should parse package source types from docs examples", () => {
            const parseNpm = (source) => {
                const parsed = packageManager.parseSource(source);
                if (parsed.type !== "npm") {
            ...
            expect(packageManager.parseSource("git:github.com/user/repo@v1").type).toBe("git");
            expect(packageManager.parseSource("https://github.com/user/repo@v1").type).toBe("git");
            expect(packageManager.parseSource("git:git@github.com:user/repo@v1").type).toBe("git");
            expect(packageManager.parseSource("ssh://git@github.com/user/repo@v1").type).toBe("git");
            expect(packageManager.parseSource("/absolute/path/to/package").type).toBe("local");
            expect(packageManager.parseSource("./relative/path/to/package").type).toBe("local");
            expect(packageManager.parseSource("../relative/path/to/package").type).toBe("local");
        });
        it("should never parse dot-relative paths as git", () => {
            const dotSlash = packageManager.parseSource("./packages/agent-timers");
            expect(dotSlash.type).toBe("local");
            expect(dotSlash.path).toBe("./packages/agent-timers");
            const dotDotSlash = packageManager.parseSource("../packages/agent-timers");
    ...
    describe("git install paths", () => {
        it("should reject paths outside git install roots", () => {
            ...
            for (const scope of ["user", "project", "temporary"]) {
                expect(() => managerWithInternals.getGitInstallPath(traversalSource, scope)).toThrow("outside package install root");
            }
        });
    });
    ...
    describe("HTTPS git URL parsing (old behavior)", () => {
        it("should parse HTTPS GitHub URLs correctly", async () => {
            const parsed = packageManager.parseSource("https://github.com/user/repo");
        ...
        it("should parse HTTPS URLs with git: prefix", async () => {
            const parsed = packageManager.parseSource("git:https://github.com/user/repo");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("github.com");
            expect(parsed.path).toBe("user/repo");
        });
        it("should parse HTTPS URLs with ref", async () => {
            const parsed = packageManager.parseSource("https://github.com/user/repo@v1.2.3");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("github.com");
            expect(parsed.path).toBe("user/repo");
        ...
        it("should parse host/path shorthand only with git: prefix", async () => {
            const parsed = packageManager.parseSource("git:github.com/user/repo");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("github.com");
            expect(parsed.path).toBe("user/repo");
        });
        it("should treat host/path shorthand as local without git: prefix", async () => {
            const parsed = packageManager.parseSource("github.com/user/repo");
            expect(parsed.type).toBe("local");
        });
        it("should parse HTTPS URLs with .git suffix", async () => {
            const parsed = packageManager.parseSource("https://github.com/user/repo.git");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("github.com");
            expect(parsed.path).toBe("user/repo");
        });
        it("should parse GitLab HTTPS URLs", async () => {
            const parsed = packageManager.parseSource("https://gitlab.com/user/repo");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("gitlab.com");
            expect(parsed.path).toBe("user/repo");
        });
        it("should parse Bitbucket HTTPS URLs", async () => {
            const parsed = packageManager.parseSource("https://bitbucket.org/user/repo");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("bitbucket.org");
            expect(parsed.path).toBe("user/repo");
        });
        it("should parse Codeberg HTTPS URLs", async () => {
            const parsed = packageManager.parseSource("https://codeberg.org/user/repo");
            expect(parsed.type).toBe("git");
            expect(parsed.host).toBe("codeberg.org");
            expect(parsed.path).toBe("user/repo");
        });
        it("should generate correct package identity for protocol and git:-prefixed URLs", async () => {
            const identity1 = packageManager.getPackageIdentity("https://github.com/user/repo");
            const identity2 = packageManager.getPackageIdentity("https://github.com/user/repo@v1.0.0");
            const identity3 = packageManager.getPackageIdentity("git:github.com/user/repo");
            const identity4 = packageManager.getPackageIdentity("https://github.com/user/repo.git");
        ...
        it("should deduplicate git URLs with different supported formats", async () => {
            ...
            const id1 = packageManager.getPackageIdentity("https://github.com/user/repo");
            const id2 = packageManager.getPackageIdentity("git:github.com/user/repo");
            const id3 = packageManager.getPackageIdentity("https://github.com/user/repo.git");
            expect(id1).toBe(id2);
            expect(id2).toBe(id3);
        });
        it("should handle HTTPS URLs with refs in resolve", async () => {
            // This tests that the ref is properly extracted and stored
            const parsed = packageManager.parseSource("https://github.com/user/repo@main");
            expect(parsed.ref).toBe("main");
            expect(parsed.pinned).toBe(true);
            const parsed2 = packageManager.parseSource("https://github.com/user/repo@feature/branch");
    ...
    describe("pattern filtering in pi manifest", () => {
        ...
        it("should support glob patterns in manifest skills", async () => {
            ...
            writeFileSync(join(pkgDir, "skills/good-skill", "SKILL.md"), "---\nname: good-skill\ndescription: Good\n---\nContent");
        ...
        it("should expand positive glob manifest entries before collecting skills", async () => {
            ...
            writeFileSync(join(pkgDir, "plugins/pdf-to-markdown/skills/pdf-to-markdown", "SKILL.md"), "---\nname: pdf-to-markdown\ndescription: PDF to Markdown\n---\nContent");
            writeFileSync(join(pkgDir, "plugins/nutrient-dws/skills/document-processor-api", "SKILL.md"), "---\nname: document-processor-api\ndescription: DWS\n---\nContent");
            writeFileSync(join(pkgDir, "package.json"), JSON.stringify({
    ...
    describe("package deduplication", () => {
        ...
        it("should dedupe SSH and HTTPS URLs for same repo", async () => {
            ...
            const httpsIdentity = packageManager.getPackageIdentity(httpsUrl);
            const sshIdentity = packageManager.getPackageIdentity(sshUrl);
        ...
        it("should dedupe SSH URL with ssh:// protocol and git@ format", async () => {
            ...
            const sshProtocolIdentity = packageManager.getPackageIdentity(sshProtocol);
            const gitAtIdentity = packageManager.getPackageIdentity(gitAt);
        ...
        it("should dedupe all supported URL formats for same repo", async () => {
            ...
            const identities = urls.map((url) => packageManager.getPackageIdentity(url));
        ...
        it("should keep different repos separate (HTTPS vs SSH)", async () => {
            ...
            const id1 = packageManager.getPackageIdentity(repo1Https);
            const id2 = packageManager.getPackageIdentity(repo2Ssh);
    ...
    describe("multi-file extension discovery (issue #1102)", () => {
        it("should only load index.ts from subdirectories, not helper modules", async () => {
            ...
            // Main entry point
            writeFileSync(join(pkgDir, "extensions", "subagent", "index.ts"), `import { helper } from "./agents.ts";
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`);
            ...
            writeFileSync(join(pkgDir, "extensions", "subagent", "agents.ts"), `export function helper() { return "helper"; }`);
        ...
        it("should respect package.json pi.extensions manifest in subdirectories", async () => {
            ...
            writeFileSync(join(pkgDir, "extensions", "custom", "package.json"), JSON.stringify({
                pi: {
                    extensions: ["./main.ts"],
                },
            }));
        ...
        it("should handle mixed top-level files and subdirectories", async () => {
            ...
            writeFileSync(join(pkgDir, "extensions", "complex", "index.ts"), "import { a } from './a.ts'; export default function(api) {}");
    ...
    describe("offline mode and network timeouts", () => {
        it("should update npm range packages using the configured spec", async () => {
            ...
            const runCommandCaptureSpy = vi
                .spyOn(packageManager, "runCommandCapture")
                .mockResolvedValue('["1.0.0","1.2.0"]');
            const runCommandSpy = vi.spyOn(packageManager, "runCommand").mockResolvedValue(undefined);
            await packageManager.update("npm:example");
            expect(runCommandCaptureSpy).toHaveBeenCalledWith("npm", ["view", "example@^1.0.0", "version", "--json"], expect.objectContaining({ cwd: tempDir, timeoutMs: expect.any(Number) }));
            expect(runCommandSpy).toHaveBeenCalledWith("npm", ["install", "example@^1.0.0", "--prefix", join(tempDir, ".pi", "npm"), "--legacy-peer-deps"], undefined);
        });
        ...
        it("should migrate legacy user npm installs into the managed npm root during update", async () => {
            ...
            vi.spyOn(packageManager, "getGlobalNpmRoot").mockReturnValue(legacyRoot);
            const runCommandCaptureSpy = vi.spyOn(packageManager, "runCommandCapture").mockResolvedValue('"1.0.0"');
            const runCommandSpy = vi
                .spyOn(packageManager, "runCommand")
                .mockImplementation(async (...callArgs) => {
                const [command, args] = callArgs;
        ...
        it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
            ...
            writeFileSync(join(projectCurrentPath, "package.json"), JSON.stringify({ name: "project-current", version: "1.0.0" }));
            settingsManager.setPackages([
                "npm:user-old",
                "npm:user-current",
                "npm:user-unknown",
            ...
            const updateGitSpy = vi.spyOn(packageManager, "updateGit").mockImplementation(async () => {
                activeGitUpdates += 1;
            ...
            expect(runCommandSpy).toHaveBeenNthCalledWith(1, "npm", [
                "install",
                "user-old@latest",
                "user-unknown@latest",
                "--prefix",
            ...
            expect(runCommandSpy).toHaveBeenNthCalledWith(2, "npm", [
                "install",
                "project-old@latest",
        ...
        it("should suggest npm source prefixes for update lookups", async () => {
            ...
            await expect(packageManager.update("example")).rejects.toThrow("No matching package found for example. Did you mean npm:example?");
        });
        it("should suggest git source prefixes for update lookups", async () => {
            ...
            await expect(packageManager.update("github.com/example/repo")).rejects.toThrow("No matching package found for github.com/example/repo. Did you mean git:github.com/example/repo?");
        });
        it("should skip installing missing package sources when offline", async () => {
            process.env.NYAN_OFFLINE = "1";
            settingsManager.setProjectPackages(["npm:missing-package", "git:github.com/example/missing-repo"]);
            const installParsedSourceSpy = vi.spyOn(packageManager, "installParsedSource");
            const result = await packageManager.resolve();
        ...
        it("should skip refreshing temporary git sources when offline", async () => {
            ...
            const parsedGitSource = packageManager.parseSource(gitSource);
            const installedPath = packageManager.getGitInstallPath(parsedGitSource, "temporary");
            mkdirSync(join(installedPath, "extensions"), { recursive: true });
            writeFileSync(join(installedPath, "extensions", "index.ts"), "export default function() {};");
            const refreshTemporaryGitSourceSpy = vi.spyOn(packageManager, "refreshTemporaryGitSource");
        ...
        it("should not run npm view during resolve for installed unpinned packages", async () => {
            ...
            writeFileSync(join(installedPath, "extensions", "index.ts"), "export default function() {};");
            settingsManager.setProjectPackages(["npm:example@^1.0.0"]);
            const runCommandCaptureSpy = vi.spyOn(packageManager, "runCommandCapture");
            const result = await packageManager.resolve();
            expect(result.extensions.some((r) => pathEndsWith(r.path, "extensions/index.ts") && r.enabled)).toBe(true);
            expect(runCommandCaptureSpy).not.toHaveBeenCalled();
        });
        it("should reinstall pinned npm packages when installed version does not match", async () => {
            ...
            const installParsedSourceSpy = vi
                .spyOn(packageManager, "installParsedSource")
        ...
        it("should report updates for installed unpinned npm packages", async () => {
            ...
            writeFileSync(join(installedPath, "package.json"), JSON.stringify({ name: "example", version: "1.0.0" }));
            settingsManager.setProjectPackages(["npm:example"]);
            vi.spyOn(packageManager, "runCommandCapture").mockResolvedValue('"1.2.3"');
            const updates = await packageManager.checkForAvailableUpdates();
            expect(updates).toEqual([
                {
                    source: "npm:example",
        ...
        it("should skip pinned packages when checking for updates", async () => {
            ...
            const parsedGitSource = packageManager.parseSource("git:github.com/example/repo@v1");
            const installedGitPath = packageManager.getGitInstallPath(parsedGitSource, "project");
            ...
            const runCommandCaptureSpy = vi.spyOn(packageManager, "runCommandCapture");
            const gitUpdateSpy = vi.spyOn(packageManager, "gitHasAvailableUpdate");
            const updates = await packageManager.checkForAvailableUpdates();
            expect(updates).toEqual([]);
            expect(runCommandCaptureSpy).not.toHaveBeenCalled();
        ...
        it("should use npm view to fetch latest version", async () => {
            const runCommandCaptureSpy = vi.spyOn(packageManager, "runCommandCapture").mockResolvedValue('"1.2.3"');
            const latest = await packageManager.getLatestNpmVersion("example");
            expect(latest).toBe("1.2.3");
            expect(runCommandCaptureSpy).toHaveBeenCalledTimes(1);
            expect(runCommandCaptureSpy).toHaveBeenCalledWith("npm", ["view", "example", "version", "--json"], expect.objectContaining({ cwd: tempDir, timeoutMs: expect.any(Number) }));
        });
        it("should use npmCommand argv for npm update checks", async () => {
            ...
            const runCommandCaptureSpy = vi.spyOn(packageManager, "runCommandCapture").mockResolvedValue('"1.2.3"');
            const latest = await packageManager.getLatestNpmVersion("@scope/pkg");
            expect(latest).toBe("1.2.3");
            expect(runCommandCaptureSpy).toHaveBeenCalledWith("mise", ["exec", "node@20", "--", "npm", "view", "@scope/pkg", "version", "--json"], expect.objectContaining({ cwd: tempDir }));
        });
