...
const fail = { stdout: "", stderr: "error", code: 1, killed: false };
/** Standard exec results for a clean repo tracking origin/main, not in a merge. */
function withUpstream(results) {
...
function setup(cwd, execResults) {
    let handler;
...
describe("git-merge-and-resolve example", () => {
    ...
    it("re-sends conflicts when in an unfinished merge", async () => {
        ...
        const message = sendUserMessage.mock.calls[0][0];
    ...
    it("skips when fetch fails", async () => {
        const cwd = createTempDir();
        const results = withUpstream(new Map());
...
//# sourceMappingURL=git-merge-and-resolve-extension.test.js.map