...
import { registerOAuthProvider } from "@nyan-works/nyan-ai/oauth";
import lockfile from "proper-lockfile";
import { afterEach, beforeEach, describe, expect, test, vi } from "vitest";
import { AuthStorage } from "../src/core/auth-storage.js";
import { clearConfigValueCache, resolveConfigValueUncached } from "../src/core/resolve-config-value.js";
...
describe("AuthStorage", () => {
    ...
    describe("API key resolution", () => {
        ...
        test("apiKey with $ prefix resolves to env value", async () => {
            const originalEnv = process.env.TEST_AUTH_ANYAN_KEY_12345;
            process.env.TEST_AUTH_ANYAN_KEY_12345 = "env-api-key-value";
            try {
                writeAuthJson({
                    anthropic: { type: "api_key", key: "$TEST_AUTH_ANYAN_KEY_12345" },
                });
                authStorage = AuthStorage.create(authJsonPath);
                const apiKey = await authStorage.getApiKey("anthropic");
                ...
                if (originalEnv === undefined) {
                    delete process.env.TEST_AUTH_ANYAN_KEY_12345;
                }
                else {
                    process.env.TEST_AUTH_ANYAN_KEY_12345 = originalEnv;
                }
            }
        });
        test("apiKey env bag takes precedence over process.env", async () => {
            const originalEnv = process.env.TEST_AUTH_SCOPED_ANYAN_KEY_12345;
            process.env.TEST_AUTH_SCOPED_ANYAN_KEY_12345 = "process-env-value";
            try {
                writeAuthJson({
                    anthropic: {
                        type: "api_key",
                        key: "$TEST_AUTH_SCOPED_ANYAN_KEY_12345",
                        env: { TEST_AUTH_SCOPED_ANYAN_KEY_12345: "credential-env-value" },
                    },
                });
                authStorage = AuthStorage.create(authJsonPath);
                ...
                expect(authStorage.getProviderEnv("anthropic")).toEqual({
                    TEST_AUTH_SCOPED_ANYAN_KEY_12345: "credential-env-value",
                });
            }
            finally {
                if (originalEnv === undefined) {
                    delete process.env.TEST_AUTH_SCOPED_ANYAN_KEY_12345;
                }
                else {
                    process.env.TEST_AUTH_SCOPED_ANYAN_KEY_12345 = originalEnv;
                }
            }
        });
        test("apiKey with braced env syntax resolves to env value", async () => {
            const originalEnv = process.env.TEST_AUTH_BRACED_ANYAN_KEY_12345;
            process.env.TEST_AUTH_BRACED_ANYAN_KEY_12345 = "braced-env-api-key-value";
            const bracedKey = "$" + "{TEST_AUTH_BRACED_ANYAN_KEY_12345}";
            try {
                writeAuthJson({
                ...
                if (originalEnv === undefined) {
                    delete process.env.TEST_AUTH_BRACED_ANYAN_KEY_12345;
                }
                else {
                    process.env.TEST_AUTH_BRACED_ANYAN_KEY_12345 = originalEnv;
                }
            }
        });
        ...
        test("apiKey with $$ prefix escapes a leading dollar", async () => {
            ...
            authStorage = AuthStorage.create(authJsonPath);
            const apiKey = await authStorage.getApiKey("anthropic");
            expect(apiKey).toBe("$TEST_AUTH_ANYAN_KEY_12345");
        });
        test("apiKey with $! escapes a literal bang and still interpolates later env refs", async () => {
            ...
            try {
                writeAuthJson({
                    anthropic: { type: "api_key", key: "$!literal-$TEST_AUTH_ANYAN_KEY_12345" },
                });
                authStorage = AuthStorage.create(authJsonPath);
                const apiKey = await authStorage.getApiKey("anthropic");
    ...
    describe("persistence semantics", () => {
        test("set preserves unrelated external edits", () => {
            ...
            const updated = JSON.parse(readFileSync(authJsonPath, "utf-8"));
