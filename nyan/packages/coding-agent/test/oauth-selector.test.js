...
const originalOpenAiApiKey = process.env.OPENAI_ANYAN_KEY;
describe("OAuthSelectorComponent", () => {
    ...
    afterEach(() => {
        if (originalOpenAiApiKey === undefined) {
            delete process.env.OPENAI_ANYAN_KEY;
        }
        else {
            process.env.OPENAI_ANYAN_KEY = originalOpenAiApiKey;
        }
    });
    ...
    it("shows stored OAuth auth distinctly in the API key selector", () => {
        ...
        const selector = new OAuthSelectorComponent("login", authStorage, [{ id: "anthropic", name: "Anthropic", authType: "api_key" }], () => { }, () => { });
        const output = stripAnsi(selector.render(120).join("\n"));
        expect(output).toContain("Anthropic");
        expect(output).toContain("subscription configured");
    });
    it("shows environment API key auth as configured", () => {
        process.env.OPENAI_ANYAN_KEY = "test-openai-key";
        const authStorage = AuthStorage.inMemory();
        const selector = new OAuthSelectorComponent("login", authStorage, [{ id: "openai", name: "OpenAI", authType: "api_key" }], () => { }, () => { });
        const output = stripAnsi(selector.render(120).join("\n"));
        expect(output).toContain("OpenAI");
        expect(output).toContain("✓ env: OPENAI_ANYAN_KEY");
        expect(output).not.toContain("unconfigured");
    });
    it("shows custom provider environment API key auth from status resolver", () => {
        const authStorage = AuthStorage.inMemory();
        const selector = new OAuthSelectorComponent("login", authStorage, [{ id: "ollama", name: "ollama", authType: "api_key" }], () => { }, () => { }, () => ({ configured: true, source: "environment", label: "OLLAMA_ANYAN_KEY" }));
        const output = stripAnsi(selector.render(120).join("\n"));
        expect(output).toContain("ollama");
        expect(output).toContain("✓ env: OLLAMA_ANYAN_KEY");
        expect(output).not.toContain("unconfigured");
    });
    it("shows models.json API key auth as configured", () => {
        const authStorage = AuthStorage.inMemory();
        const selector = new OAuthSelectorComponent("login", authStorage, [{ id: "local-proxy", name: "local-proxy", authType: "api_key" }], () => { }, () => { }, () => ({ configured: true, source: "models_json_key" }));
        const output = stripAnsi(selector.render(120).join("\n"));
    ...
    it("shows models.json command auth as configured", () => {
        const authStorage = AuthStorage.inMemory();
        const selector = new OAuthSelectorComponent("login", authStorage, [{ id: "op-proxy", name: "op-proxy", authType: "api_key" }], () => { }, () => { }, () => ({ configured: true, source: "models_json_command" }));
        const output = stripAnsi(selector.render(120).join("\n"));
...
//# sourceMappingURL=oauth-selector.test.js.map