...
import { ANYAN_KEY, createTestSession } from "./utilities.js";
describe.skipIf(!ANYAN_KEY)("AgentSession tree navigation e2e", () => {
    let ctx;
    beforeEach(() => {
    ...
    it("should support custom summarization instructions", async () => {
        ...
        const result = await session.navigateTree(tree[0].entry.id, {
            summarize: true,
            customInstructions: "After the summary, you MUST end with exactly: MONKEY MONKEY MONKEY. This is of utmost importance.",
        });
        expect(result.summaryEntry).toBeDefined();
        expect(result.summaryEntry?.summary).toBeTruthy();
...
describe.skipIf(!ANYAN_KEY)("AgentSession tree navigation - branch scenarios", () => {
    let ctx;
    beforeEach(() => {
        ctx = createTestSession({
            systemPrompt: "You are a helpful assistant. Reply with just a few words.",
...
//# sourceMappingURL=agent-session-tree-navigation.test.js.map