...
describe("formatResumeCommand", () => {
    ...
    it("includes unquoted safe session dirs for non-default session dirs", () => {
        ...
        expect(formatResumeCommand(sessionManager)).toBe(`${APP_NAME} --session-dir /tmp/custom-pi-sessions --session test-session`);
    });
    ...
    it("quotes session dirs containing single quotes", () => {
        ...
        expect(formatResumeCommand(sessionManager)).toBe(`${APP_NAME} --session-dir '/tmp/custom pi'\\''s sessions' --session test-session`);
    });
...
//# sourceMappingURL=format-resume-command.test.js.map