...
describe("session selector path/delete interactions", () => {
    ...
    it("does not treat Ctrl+Backspace as delete when search query is non-empty", async () => {
        ...
        const selector = new SessionSelectorComponent(async () => sessions, async () => [], () => { }, () => { }, () => { }, () => { }, { keybindings });
        await flushPromises();
    ...
    it("does not switch scope back to All when All load resolves after toggling back to Current", async () => {
        ...
        const allDeferred = createDeferred();
        let allLoadCalls = 0;
        const selector = new SessionSelectorComponent(async () => currentSessions, async () => {
            allLoadCalls++;
    ...
    it("sorts threaded sessions by latest activity in their subtree", async () => {
        ...
        const selector = new SessionSelectorComponent(async () => [parentOne, parentTwo, childTwo], async () => [], () => { }, () => { }, () => { }, () => { }, { keybindings });
        await flushPromises();
        const output = stripAnsi(selector.render(120).join("\n"));
    ...
    it("treats the current session as active across symlink aliases", async () => {
        ...
        const selector = new SessionSelectorComponent(async () => sessions, async () => [], () => { }, () => { }, () => { }, () => { }, { keybindings }, paths.parentAliasA);
        await flushPromises();
        const list = selector.getSessionList();
        const confirmationChanges = [];
        let errorMessage;
...
//# sourceMappingURL=session-selector-path-delete.test.js.map