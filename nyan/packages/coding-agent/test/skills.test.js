...
describe("skills", () => {
    describe("loadSkillsFromDir", () => {
        ...
        it("should warn when name exceeds 64 characters", () => {
            ...
            expect(skills).toHaveLength(1);
            expect(diagnostics.some((d) => d.message.includes("exceeds 64 characters"))).toBe(true);
        });
        ...
        it("should warn and skip skill when YAML frontmatter is invalid", () => {
            ...
            expect(skills).toHaveLength(0);
            expect(diagnostics.some((d) => d.message.includes("at line"))).toBe(true);
        });
        ...
        it("should warn when name contains consecutive hyphens", () => {
            ...
            expect(skills).toHaveLength(1);
            expect(diagnostics.some((d) => d.message.includes("consecutive hyphens"))).toBe(true);
        });
        ...
        it("should parse disable-model-invocation frontmatter field", () => {
            ...
            expect(diagnostics.some((d) => d.message.includes("unknown frontmatter field"))).toBe(false);
        });
    ...
    describe("loadSkills with options", () => {
        ...
        it("should expand ~ in skillPaths", () => {
            const homeSkillsDir = join(homedir(), ".nyan/agent/skills");
            const { skills: withTilde } = loadSkills({
                agentDir: emptyAgentDir,
                cwd: emptyCwd,
                skillPaths: ["~/.nyan/agent/skills"],
                includeDefaults: true,
            });
