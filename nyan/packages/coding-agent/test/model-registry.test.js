...
import { getApiProvider } from "@nyan-works/nyan-ai/compat";
import { getOAuthProvider } from "@nyan-works/nyan-ai/oauth";
...
describe("ModelRegistry", () => {
    ...
    function providerConfig(baseUrl, models, api = "anthropic-messages") {
        return {
            baseUrl,
            apiKey: "test-key",
            api: api,
    ...
    function writeModelsJson(providers) {
        writeFileSync(modelsJsonPath, JSON.stringify({ providers }));
    }
    function getModelsForProvider(registry, provider) {
        return registry.getAll().filter((m) => m.provider === provider);
    }
    function toShPath(value) {
        return value.replace(/\\/g, "/").replace(/"/g, '\\"');
    }
    /** Create a baseUrl-only override (no custom models) */
    function overrideConfig(baseUrl, headers) {
        return { baseUrl, ...(headers && { headers }) };
    }
    /** Write raw providers config (for mixed override/replacement scenarios) */
    function writeRawModelsJson(providers) {
        writeFileSync(modelsJsonPath, JSON.stringify({ providers }));
    }
    const openAiModel = {
    ...
    describe("baseUrl override (no custom models)", () => {
        ...
        test("can mix baseUrl override and models merge", () => {
            writeRawModelsJson({
                ...
                google: providerConfig("https://google-proxy.example.com/v1", [{ id: "gemini-custom" }], "google-generative-ai"),
            });
            const registry = ModelRegistry.create(authStorage, modelsJsonPath);
    ...
    describe("custom models merge behavior", () => {
        ...
        test("custom model with same id replaces built-in model by id", () => {
            writeModelsJson({
                openrouter: providerConfig("https://my-proxy.example.com/v1", [{ id: "anthropic/claude-sonnet-4" }], "openai-completions"),
            });
            const registry = ModelRegistry.create(authStorage, modelsJsonPath);
        ...
        test("provider-level compat applies to custom models", () => {
            ...
            const registry = ModelRegistry.create(authStorage, modelsJsonPath);
            const compat = registry.find("demo", "demo-model")?.compat;
        ...
        test("modelOverrides still apply when provider also defines models", () => {
            ...
            expect(models.some((m) => m.id === "anthropic/claude-sonnet-4" && m.name === "Overridden Built-in Sonnet")).toBe(true);
        });
    ...
    describe("modelOverrides (per-model customization)", () => {
        ...
        test("refresh() picks up model override changes", () => {
            ...
            const registry = ModelRegistry.create(authStorage, modelsJsonPath);
            expect(getModelsForProvider(registry, "openrouter").find((m) => m.id === "anthropic/claude-sonnet-4")?.name).toBe("First Name");
            // Update and refresh
            writeRawModelsJson({
                openrouter: {
                    modelOverrides: {
            ...
            registry.refresh();
            expect(getModelsForProvider(registry, "openrouter").find((m) => m.id === "anthropic/claude-sonnet-4")?.name).toBe("Second Name");
        });
        test("removing model override restores built-in values", () => {
            ...
            const registry = ModelRegistry.create(authStorage, modelsJsonPath);
            const customName = getModelsForProvider(registry, "openrouter").find((m) => m.id === "anthropic/claude-sonnet-4")?.name;
            expect(customName).toBe("Custom Name");
            // Remove override and refresh
            writeRawModelsJson({});
            registry.refresh();
            const restoredName = getModelsForProvider(registry, "openrouter").find((m) => m.id === "anthropic/claude-sonnet-4")?.name;
    ...
    describe("dynamic provider lifecycle", () => {
        ...
        test("stored API key env propagates to request auth and resolves headers", async () => {
            authStorage.set("cloudflare-ai-gateway", {
                type: "api_key",
                key: "$CLOUDFLARE_ANYAN_KEY",
                env: {
                    CLOUDFLARE_ANYAN_KEY: "stored-cf-token",
        ...
        test("failed registerProvider does not persist invalid streamSimple config", () => {
            const registry = ModelRegistry.create(authStorage, modelsJsonPath);
            expect(() => registry.registerProvider("broken-provider", {
                streamSimple: (() => {
        ...
        describe("dynamic provider override persistence", () => {
            ...
            test("models-only custom provider registration survives refresh", () => {
                const registry = ModelRegistry.create(authStorage, modelsJsonPath);
                registry.registerProvider("custom-provider", providerConfig("https://custom.test/v1", [{ id: "custom-a" }, { id: "custom-b" }], "openai-completions"));
                registry.refresh();
            ...
            test("baseUrl-only override keeps custom provider models after refresh", () => {
                ...
                expect(getModelsForProvider(registry, "custom-provider").every((m) => m.baseUrl === "https://proxy.test/custom")).toBe(true);
            });
    ...
    describe("API key resolution", () => {
        ...
        function providerWithApiKey(apiKey) {
            return {
                baseUrl: "https://example.com/v1",
                apiKey,
                api: "anthropic-messages",
        ...
        test("apiKey with $ prefix resolves to env value", async () => {
            const originalEnv = process.env.TEST_ANYAN_KEY_12345;
            process.env.TEST_ANYAN_KEY_12345 = "env-api-key-value";
            try {
                writeRawModelsJson({
                    "custom-provider": providerWithApiKey("$TEST_ANYAN_KEY_12345"),
                });
                const registry = ModelRegistry.create(authStorage, modelsJsonPath);
                const apiKey = await registry.getApiKeyForProvider("custom-provider");
                ...
                if (originalEnv === undefined) {
                    delete process.env.TEST_ANYAN_KEY_12345;
                }
                else {
                    process.env.TEST_ANYAN_KEY_12345 = originalEnv;
                }
            }
        });
        test("apiKey with braced env syntax resolves to env value", async () => {
            const originalEnv = process.env.TEST_BRACED_ANYAN_KEY_12345;
            process.env.TEST_BRACED_ANYAN_KEY_12345 = "braced-env-api-key-value";
            const bracedKey = "$" + "{TEST_BRACED_ANYAN_KEY_12345}";
            try {
                writeRawModelsJson({
                ...
                if (originalEnv === undefined) {
                    delete process.env.TEST_BRACED_ANYAN_KEY_12345;
                }
                else {
                    process.env.TEST_BRACED_ANYAN_KEY_12345 = originalEnv;
                }
            }
        });
        ...
        test("apiKey with $$ prefix escapes a leading dollar", async () => {
            ...
            const registry = ModelRegistry.create(authStorage, modelsJsonPath);
            const apiKey = await registry.getApiKeyForProvider("custom-provider");
            expect(apiKey).toBe("$TEST_ANYAN_KEY_12345");
        });
        test("apiKey with $! escapes a literal bang and still interpolates later env refs", async () => {
            ...
            try {
                writeRawModelsJson({
                    "custom-provider": providerWithApiKey("$!literal-$TEST_ANYAN_KEY_12345"),
                });
                const registry = ModelRegistry.create(authStorage, modelsJsonPath);
                const apiKey = await registry.getApiKeyForProvider("custom-provider");
        ...
        describe("request-time resolution", () => {
            ...
            test("provider auth status reports apiKey environment variables from models.json", () => {
                const envVarName = "TEST_ANYAN_KEY_STATUS_TEST_98765";
            ...
            test("provider auth status reports interpolated apiKey environment variables", () => {
                const envVarNameA = "TEST_ANYAN_KEY_STATUS_PART_A_98765";
                const envVarNameB = "TEST_ANYAN_KEY_STATUS_PART_B_98765";
            ...
            test("missing explicit env apiKey keeps provider unavailable", () => {
                const envVarName = "TEST_ANYAN_KEY_MISSING_TEST_98765";
                const originalEnv = process.env[envVarName];
                delete process.env[envVarName];
                try {
                    writeRawModelsJson({
            ...
            test("environment variables are not cached (changes are picked up)", async () => {
                const envVarName = "TEST_ANYAN_KEY_CACHE_TEST_98765";
                const originalEnv = process.env[envVarName];
                try {
