...
function parseOutputLines(outputLines) {
    return outputLines
        .flatMap((line) => line.split("\n"))
        .filter((line) => line.trim().length > 0)
        .map((line) => JSON.parse(line));
}
function getPromptResponses(outputLines, id) {
    return parseOutputLines(outputLines).filter((record) => record.id === id && record.type === "response" && record.command === "prompt");
}
function sleep(ms) {
...
//# sourceMappingURL=rpc-prompt-response-semantics.test.js.map