...
import { EventStream, getModel, } from "@nyan-works/nyan-ai/compat";
...
describe("AgentSession concurrent prompt guard", () => {
    ...
    afterEach(async () => {
        delete globalThis.testExtensionApi;
        delete globalThis.testCommandRuns;
        if (session) {
            session.dispose();
        }
    ...
    it("should throw when prompt() called while streaming", async () => {
        ...
        await expect(session.prompt("Second message")).rejects.toThrow("Agent is already processing. Specify streamingBehavior ('steer' or 'followUp') to queue the message.");
        // Cleanup
        await session.abort();
    ...
    it("should queue extension-origin steering messages while streaming", async () => {
        ...
        const extensionsResult = await createTestExtensionsResult([
            (pi) => {
                globalThis.testExtensionApi = pi;
            },
            (pi) => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 10));
        expect(session.isStreaming).toBe(true);
        const pi = globalThis.testExtensionApi;
    ...
    it("should wait for queued agent events before emitting tool_call", async () => {
        ...
        const tool = {
            ...
            execute: async (_toolCallId, params) => {
                const q = typeof params === "object" && params !== null && "q" in params
                    ? String(params.q)
                    : "";
                return {
                    content: [{ type: "text", text: `result:${q}` }],
                    details: {},
                };
            },
        };
...
//# sourceMappingURL=agent-session-concurrent.test.js.map