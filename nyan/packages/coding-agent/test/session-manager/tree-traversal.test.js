...
describe("SessionManager append and tree traversal", () => {
    describe("append operations", () => {
        ...
        it("appendCustomEntry integrates into tree", () => {
            ...
            const _msg2Id = session.appendMessage(assistantMsg("response"));
            const entries = session.getEntries();
            const customEntry = entries.find((e) => e.type === "custom");
    ...
    describe("getEntry", () => {
        ...
        it("returns entry by id", () => {
            ...
            if (entry2?.type === "message" && entry2.message.role === "assistant") {
                expect(entry2.message.content[0].text).toBe("second");
            }
        });
    });
    describe("buildSessionContext with branches", () => {
        it("returns messages from current branch only", () => {
            ...
            expect(ctx.messages[0].content).toBe("msg1");
            expect(ctx.messages[1].content[0].text).toBe("msg2");
            expect(ctx.messages[2].content[0].text).toBe("msg4-branch");
        });
    });
});
...
//# sourceMappingURL=tree-traversal.test.js.map