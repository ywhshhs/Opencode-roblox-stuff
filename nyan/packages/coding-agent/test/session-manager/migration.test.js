...
import { migrateSessionEntries } from "../../src/core/session-manager.js";
describe("migrateSessionEntries", () => {
    ...
    it("should be idempotent (skip already migrated)", () => {
        ...
        expect(entries[1].id).toBe("abc12345");
        expect(entries[2].id).toBe("def67890");
        expect(entries[2].parentId).toBe("abc12345");
    });
});
