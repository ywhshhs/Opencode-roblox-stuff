...
function compaction(id, parentId, summary, firstKeptEntryId) {
    return {
        type: "compaction",
        id,
        parentId,
...
function branchSummary(id, parentId, summary, fromId) {
...
describe("buildSessionContext", () => {
    describe("trivial cases", () => {
        ...
        it("single user message", () => {
            const entries = [msg("1", null, "user", "hello")];
        ...
        it("tracks model from assistant message", () => {
            const entries = [msg("1", null, "user", "hello"), msg("2", "1", "assistant", "hi")];
            const ctx = buildSessionContext(entries);
    ...
    describe("with compaction", () => {
        it("includes summary before kept messages", () => {
            ...
            expect(ctx.messages[0].summary).toContain("Summary of first two turns");
            expect(ctx.messages[1].content).toBe("second");
            expect(ctx.messages[2].content[0].text).toBe("response2");
            expect(ctx.messages[3].content).toBe("third");
            expect(ctx.messages[4].content[0].text).toBe("response3");
        });
        it("handles compaction keeping from first message", () => {
            ...
            expect(ctx.messages).toHaveLength(4);
            expect(ctx.messages[0].summary).toContain("Empty summary");
        });
        it("multiple compactions uses latest", () => {
            ...
            expect(ctx.messages).toHaveLength(4);
            expect(ctx.messages[0].summary).toContain("Second summary");
        });
    });
    describe("with branches", () => {
        it("follows path to specified leaf", () => {
            ...
            expect(ctxA.messages[2].content).toBe("branch A");
            const ctxB = buildSessionContext(entries, "4");
            expect(ctxB.messages).toHaveLength(3);
            expect(ctxB.messages[2].content).toBe("branch B");
        });
        it("includes branch summary in path", () => {
            ...
            expect(ctx.messages).toHaveLength(4);
            expect(ctx.messages[2].summary).toContain("Summary of abandoned work");
            expect(ctx.messages[3].content).toBe("new direction");
        });
        it("complex tree with multiple branches and compaction", () => {
            ...
            expect(ctxMain.messages[0].summary).toContain("Compacted history");
            expect(ctxMain.messages[1].content).toBe("q2");
            expect(ctxMain.messages[2].content[0].text).toBe("r2");
            expect(ctxMain.messages[3].content).toBe("q3");
            expect(ctxMain.messages[4].content[0].text).toBe("r3");
            ...
            expect(ctxBranch.messages[0].content).toBe("start");
            expect(ctxBranch.messages[1].content[0].text).toBe("r1");
            expect(ctxBranch.messages[2].content).toBe("q2");
            expect(ctxBranch.messages[3].summary).toContain("Tried wrong approach");
            expect(ctxBranch.messages[4].content).toBe("better approach");
        });
    });
    describe("edge cases", () => {
...
//# sourceMappingURL=build-context.test.js.map