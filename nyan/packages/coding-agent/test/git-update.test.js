...
function createCommit(repoDir, filename, content, message) {
...
function getFileContent(repoDir, filename) {
    return readFileSync(join(repoDir, filename), "utf-8");
}
describe("DefaultPackageManager git update", () => {
    let tempDir;
    let remoteDir; // Simulates the "remote" repository
    let agentDir; // The agent directory where extensions are installed
    let installedDir; // The installed extension directory
    let settingsManager;
    let packageManager;
     ...
     */
    function setupRemoteAndInstall(sourceOverride) {
    ...
    describe("normal updates (no force-push)", () => {
        it("should skip reset, clean, and install when already up to date", async () => {
            ...
            expect(executedCommands).toContain("git fetch --prune --no-tags origin +refs/heads/main:refs/remotes/origin/main");
    ...
    describe("temporary git sources", () => {
        it("should refresh cached temporary git sources when resolving", async () => {
            const managerWithPaths = packageManager;
            ...
            writeFileSync(join(cachedDir, "package.json"), JSON.stringify({ pi: { extensions: ["./pi-extensions"] } }, null, 2));
...
//# sourceMappingURL=git-update.test.js.map