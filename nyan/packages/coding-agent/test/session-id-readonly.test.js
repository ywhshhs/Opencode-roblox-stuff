...
function hasSessionWithId(root, sessionId) {
    if (!existsSync(root))
        return false;
    for (const entry of readdirSync(root, { withFileTypes: true })) {
        const path = join(root, entry.name);
        if (entry.isDirectory() && hasSessionWithId(path, sessionId))
            return true;
...
async function runCli(args, setup) {
...
function writeSession(sessionDir, cwd, id) {
    writeFileSync(join(sessionDir, `${id}.jsonl`), `${JSON.stringify({ type: "session", version: 3, id, timestamp: new Date().toISOString(), cwd })}\n`);
}
describe("--session-id read-only commands", () => {
    ...
    it("rejects an existing fork target session id", async () => {
        const result = await runCli((dirs) => ["--session-dir", dirs.sessionDir, "--fork", "source-id", "--session-id", "existing-id", "-p", "hi"], (dirs) => {
...
//# sourceMappingURL=session-id-readonly.test.js.map