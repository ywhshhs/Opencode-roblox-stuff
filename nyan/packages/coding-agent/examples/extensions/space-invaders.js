 ...
 */
import { isKeyRelease, Key, matchesKey, visibleWidth } from "@nyan-works/nyan-tui";
const GAME_WIDTH = 60;
...
function createInitialState(highScore = 0, level = 1) {
    return {
...
class SpaceInvadersComponent {
    constructor(tui, onClose, onSave, savedState) {
    ...
    moveAliens() {
        ...
        if (this.state.alienDropping) {
            ...
            if ((this.state.alienDirection === 1 && maxX >= GAME_WIDTH - 3) ||
    ...
    pi.registerCommand("invaders", {
        ...
        handler: async (_args, ctx) => {
            ...
            await ctx.ui.custom((tui, _theme, _kb, done) => {
                return new SpaceInvadersComponent(tui, () => done(undefined), (state) => {
