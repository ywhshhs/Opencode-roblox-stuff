...
import { createBashTool, createEditTool, createReadTool, createWriteTool, } from "@nyan-works/nyan-coding-agent";
function sshExec(remote, command) {
...
export default function (pi) {
    ...
    pi.on("user_bash", (_event) => {
        const ssh = getSsh();
        if (!ssh)
            return; // No SSH, use local execution
    ...
    pi.on("before_agent_start", async (event) => {
        const ssh = getSsh();
        if (ssh) {
            const modified = event.systemPrompt.replace(`Current working directory: ${localCwd}`, `Current working directory: ${ssh.remoteCwd} (via SSH: ${ssh.remote})`);
