 ...
 */
function addToolGuidance(options, basePrompt) {
    const hasTool = (name) => options.selectedTools?.includes(name) ?? false;
    const parts = [];
    if (hasTool("read")) {
        parts.push("• Use the `read` tool for file contents (supports text and images).", "  - For large files, use `offset` and `limit` to read in chunks.");
    }
    if (hasTool("bash")) {
        parts.push("• Execute commands with the `bash` tool. Use it for file operations like `ls`, `find`, `grep`.");
    }
    if (hasTool("edit")) {
        parts.push("• Use the `edit` tool for precise text replacements in files. Match exact content including whitespace.");
    }
    if (hasTool("write")) {
 ...
 */
function mergeWithUserAppend(options) {
...
export default function promptCustomizer(pi) {
...
//# sourceMappingURL=prompt-customizer.js.map