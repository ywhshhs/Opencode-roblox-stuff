 ...
 */
import { VERSION } from "@nyan-works/nyan-coding-agent";
// --- PI MASCOT ---
// Based on pi_mascot.ts - the pi agent character
function getPiMascot(theme) {
    // --- COLORS ---
    // 3b1b Blue: R=80, G=180, B=230
    const piBlue = (text) => theme.fg("accent", text);
    const white = (text) => text; // Use plain white (or theme.fg("text", text))
    const black = (text) => theme.fg("dim", text); // Use dim for contrast
    // --- GLYPHS ---
    const BLOCK = "█";
