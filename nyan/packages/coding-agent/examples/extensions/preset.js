...
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@nyan-works/nyan-coding-agent";
import { Container, Key, SelectList, Text } from "@nyan-works/nyan-tui";
/**
 * Load presets from config files.
 * Project-local presets override global presets with the same name.
 */
function loadPresets(cwd) {
...
export default function presetExtension(pi) {
    let presets = {};
     ...
     */
    async function applyPreset(name, preset, ctx) {
    ...
    function buildPresetDescription(preset) {
        ...
        if (preset.instructions) {
            const truncated = preset.instructions.length > 30 ? `${preset.instructions.slice(0, 27)}...` : preset.instructions;
     ...
     */
    async function showPresetSelector(ctx) {
        const presetNames = Object.keys(presets);
        if (presetNames.length === 0) {
            ctx.ui.notify(`No presets defined. Add presets to ${join(getAgentDir(), "presets.json")} or ${join(ctx.cwd, CONFIG_DIR_NAME, "presets.json")}`, "warning");
            return;
        }
    ...
    async function cyclePreset(ctx) {
