 ...
 */
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@nyan-works/nyan-tui";
import { spawn } from "child_process";
// Global handle for toggle demo (in real code, use a more elegant pattern)
let globalToggleHandle = null;
export default function (pi) {
    ...
    pi.registerCommand("overlay-anchors", {
        ...
        handler: async (_args, ctx) => {
            ...
            let index = 0;
            while (true) {
                const result = await ctx.ui.custom((_tui, theme, _kb, done) => new AnchorTestComponent(theme, anchors[index], done), {
                    overlay: true,
    ...
    pi.registerCommand("overlay-margins", {
        ...
        handler: async (_args, ctx) => {
            ...
            let index = 0;
            while (true) {
                const result = await ctx.ui.custom((_tui, theme, _kb, done) => new MarginTestComponent(theme, configs[index], done), {
                    overlay: true,
    ...
    pi.registerCommand("overlay-stack", {
        ...
        handler: async (_args, ctx) => {
            ...
            const p1 = ctx.ui.custom((_tui, theme, _kb, done) => new StackOverlayComponent(theme, 1, "back (red border)", done), {
                overlay: true,
                overlayOptions: { anchor: "center", width: 50, offsetX: -8, offsetY: -4, maxHeight: 15 },
            });
            await sleep(400);
            ctx.ui.notify("Showing overlay 2 (middle)...", "info");
            const p2 = ctx.ui.custom((_tui, theme, _kb, done) => new StackOverlayComponent(theme, 2, "middle (green border)", done), {
                overlay: true,
                overlayOptions: { anchor: "center", width: 50, offsetX: 0, offsetY: 0, maxHeight: 15 },
            });
            await sleep(400);
            ctx.ui.notify("Showing overlay 3 (front)...", "info");
            const p3 = ctx.ui.custom((_tui, theme, _kb, done) => new StackOverlayComponent(theme, 3, "front (blue border)", done), {
                overlay: true,
                overlayOptions: { anchor: "center", width: 50, offsetX: 8, offsetY: 4, maxHeight: 15 },
            });
    ...
    pi.registerCommand("overlay-percent", {
        ...
        handler: async (_args, ctx) => {
            ...
            while (true) {
                ...
                const result = await ctx.ui.custom((_tui, theme, _kb, done) => new PercentTestComponent(theme, config, done), {
                    overlay: true,
                    overlayOptions: {
                        width: 30,
                        row: `${config.row}%`,
    ...
    pi.registerCommand("overlay-passive", {
        ...
        handler: async (_args, ctx) => {
            ...
            await ctx.ui.custom((tui, theme, _kb, done) => new PassiveDemoController(tui, theme, done), {
                overlay: true,
    ...
    pi.registerCommand("overlay-focus", {
        ...
        handler: async (_args, ctx) => {
            ctx.ui.setEditorText("");
            await ctx.ui.custom((tui, theme, _kb, done) => new FocusDemoController(tui, theme, done), {
                overlay: true,
    ...
    pi.registerCommand("overlay-streaming", {
        ...
        handler: async (_args, ctx) => {
            ctx.ui.setEditorText("");
            await ctx.ui.custom((tui, theme, _kb, done) => new StreamingInputController(tui, theme, done), {
                overlay: true,
...
class StackOverlayComponent extends BaseOverlay {
    constructor(theme, num, position, done) {
        super(theme);
        this.num = num;
    ...
    render(width) {
        ...
        const colors = ["error", "success", "accent"];
        const color = colors[(this.num - 1) % colors.length];
        const innerW = Math.max(1, width - 2);
        const border = (char) => th.fg(color, char);
        const padLine = (s) => truncateToWidth(s, innerW, "...", true);
        const lines = [];
...
class StreamingOverflowComponent extends BaseOverlay {
    ...
    startProcess() {
        ...
        this.proc.stdout?.on("data", (data) => {
            if (this.disposed)
                return; // Guard against callbacks after dispose
    ...
    render(width) {
        ...
        result.push(border("│") + padLine(canScrollUp || canScrollDown ? th.fg("dim", ` ${scrollInfo}`) : "") + border("│"));
...
class SidepanelComponent extends BaseOverlay {
    constructor(tui, theme, done) {
        super(theme);
        this.items = ["Dashboard", "Messages", "Settings", "Help", "About"];
...
class FocusDemoController extends BaseOverlay {
    ...
    focusNext(current, direction = 1) {
        ...
        if (currentOpenPosition === -1)
            throw new Error(`Panel ${current.label} is not open`);
    ...
    dismiss(panel) {
        ...
        if (!entry)
            throw new Error(`Invalid focus panel index ${currentOpenPosition}`);
    ...
    focusEntryAt(entries, index) {
        const entry = entries[index];
        if (!entry)
            throw new Error(`Invalid focus panel index ${index}`);
        entry.handle.focus();
        this.tui.requestRender();
    }
    hidePanels() {
...
class FocusPanel extends BaseOverlay {
    constructor({ theme, config, controller, }) {
        super(theme);
        this.focused = false;
        this.closed = false;
    ...
    render(width) {
        const th = this.theme;
        const innerW = Math.max(1, width - 2);
        const border = (c) => th.fg(this.focused ? this.color : "dim", c);
        const padLine = (s) => truncateToWidth(s, innerW, "...", true);
        ...
        lines.push(border(`╭${"─".repeat(innerW)}╮`));
        lines.push(border("│") +
            padLine(` ${th.fg(this.color, this.label)} ${this.focused ? th.fg("success", "FOCUSED") : th.fg("dim", "visible")}`) +
            border("│"));
        lines.push(border("│") + padLine("") + border("│"));
...
class StreamingInputController extends BaseOverlay {
    constructor(tui, theme, done) {
        ...
        this.panels = [];
        this.handles = [];
        this.focusIndex = -1; // -1 = controller focused, 0-2 = panel focused
        this.streamLines = [];
        ...
        for (let i = 0; i < 3; i++) {
            const panel = new StreamingInputPanel(theme, labels[i], colors[i], () => this.cycleFocus(), () => this.close());
    ...
    render(width) {
        const th = this.theme;
        const focusedLabel = this.focusIndex === -1
...
class StreamingInputPanel {
    constructor(theme, label, color, onTab, onClose) {
        this.handle = null;
        this.typed = "";
        this.theme = theme;
        this.label = label;
    ...
    render(width) {
        ...
        const innerW = Math.max(1, width - 2);
        const border = (c) => th.fg(this.color, c);
        const padLine = (s) => {
...
//# sourceMappingURL=overlay-qa-tests.js.map