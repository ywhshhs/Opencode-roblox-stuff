...
import { createBashTool, createEditTool, createFindTool, createGrepTool, createLsTool, createReadTool, createWriteTool, DEFAULT_MAX_BYTES, formatSize, truncateHead, truncateLine, } from "@nyan-works/nyan-coding-agent";
...
function isInsideHostPath(root, value) {
    const relativePath = path.relative(root, value);
    return relativePath === "" || (!relativePath.startsWith("..") && !path.isAbsolute(relativePath));
}
function hostPathToGuest(localCwd, hostPath) {
...
function toGuestPath(localCwd, inputPath) {
...
function createGondolinLsOps(vm, localCwd) {
    return {
    ...
    const walkDirectory = async (dir, relativeDir) => {
        ...
        for (const entry of entries) {
            ...
            if (entryStat.isDirectory()) {
                if (!(await walkDirectory(guestPath, relativePath)))
                    return false;
            }
...
function matchesToolGlob(relativePath, pattern) {
    ...
    if (normalizedPattern.includes("/")) {
        return (path.posix.matchesGlob(relativePath, normalizedPattern) ||
...
function createGondolinFindOps(vm, localCwd) {
    return {
        exists: async (filePath) => {
            try {
                await vm.fs.access(toGuestPath(localCwd, filePath));
...
async function executeGondolinGrep(vm, localCwd, params, signal) {
...
function createGondolinBashOps(vm, localCwd, shellPath) {
    return {
