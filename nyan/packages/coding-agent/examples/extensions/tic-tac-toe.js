 ...
 */
import { StringEnum } from "@nyan-works/nyan-ai";
import { matchesKey, Text, truncateToWidth, visibleWidth } from "@nyan-works/nyan-tui";
...
function boardToAscii(board, agentCursorRow, agentCursorCol) {
    ...
    const rows = board.map((row, r) => row
        .map((c, cIdx) => {
...
// for empty cells.
function cellFgCode(cell, isWin) {
    if (cell === " ")
        return undefined;
    if (isWin)
        return FG_CODE_WIN;
...
function buildCellContent(mark, lineIdx, isWin) {
    ...
    const fg = cellFgCode(mark, isWin);
...
function borderFgCode(adjacent) {
    const fgs = adjacent.map((a) => cellFgCode(a.cell, a.isWin)).filter((f) => !!f);
    if (fgs.length === 0)
        return undefined;
    const first = fgs[0];
...
function renderBoard(opts) {
    ...
    const cellAt = (r, c) => ({ cell: board[r][c], isWin: winCells.has(`${r},${c}`) });
    const isCursorCorner = (gridR, gridC) => showCursor && (gridR === cr || gridR === cr + 1) && (gridC === cc || gridC === cc + 1);
    const isCursorHSegment = (gridR, c) => showCursor && c === cc && (gridR === cr || gridR === cr + 1);
    const isCursorVBorder = (r, gridC) => showCursor && r === cr && (gridC === cc || gridC === cc + 1);
    const paintBorder = (ch, highlighted, fgCode) => {
        if (highlighted)
            return `${cursorSgr}${ch}${RESET}`;
        if (fgCode)
            return `\x1b[${fgCode};1m${ch}${RESET}`;
        return DIM(ch);
    };
    const cornerChar = (gridR, gridC) => {
        if (gridR === 0 && gridC === 0)
            return "\u250c";
        if (gridR === 0 && gridC === 3)
            return "\u2510";
    ...
    const cornerAdjacent = (gridR, gridC) => {
        const out = [];
        for (const [dr, dc] of [
            [-1, -1],
            [-1, 0],
    ...
    for (let gridR = 0; gridR <= 3; gridR++) {
        ...
        for (let gridC = 0; gridC <= 3; gridC++) {
            ...
            if (gridC < 3) {
                const adj = [];
                if (gridR > 0)
                    adj.push(cellAt(gridR - 1, gridC));
                if (gridR < 3)
                    adj.push(cellAt(gridR, gridC));
...
class TicTacToeComponent {
    constructor(tui, onClose, onUserPlay, state) {
        this.cachedLines = [];
        this.cachedWidth = 0;
        this.version = 0;
    ...
    handleInput(data) {
        ...
        if (this.state.currentTurn !== this.state.userMark)
            return true;
    ...
    render(width) {
        ...
        // Status line.
        if (this.state.status !== "playing") {
            const statusText = this.state.status === "draw"
                ? bold(yellow("Draw!"))
...
class BannerMessageComponent {
    constructor(title, details, expanded, theme) {
        this.title = title;
...
class GameOverMessageComponent {
    constructor(status, details, theme) {
        this.status = status;
        this.details = details;
        this.theme = theme;
    }
...
function reconstructState(ctx) {
    ...
    for (const entry of ctx.sessionManager.getBranch()) {
        ...
        if (msg.role !== "toolResult")
            continue;
        if (msg.toolName !== "tic_tac_toe" && msg.toolName !== "tic_tac_toe_see_board")
            continue;
...
export default function (pi) {
    ...
    const emitGameOverMessage = () => {
        const label = gameState.status === "win_X"
            ? "Player X (human) wins"
    ...
    pi.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
        ...
        const turnLabel = details?.currentTurn === "O"
    ...
    pi.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
        const details = message.details;
        const status = (details?.status ?? "draw");
...
`;
    ...
    pi.registerCommand("tic-tac-toe", {
        ...
        handler: async (_args, ctx) => {
            ...
            await ctx.ui.custom((tui, _theme, _kb, done) => {
                component = new TicTacToeComponent(tui, () => {
                    component = null;
                    gameActive = false;
                    done(undefined);
                }, (row, col) => {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            action: StringEnum(["move_up", "move_down", "move_left", "move_right", "play"], {
                description: "The single action to perform this call. Emit multiple tic_tac_toe calls in one response to string actions together.",
            }),
        }),
            ...
            switch (params.action) {
                ...
                case "play": {
                    ...
                    if (gameState.board[r][c] !== " ") {
                        ...
                        component?.updateState(gameState);
                        pi.appendEntry(SAVE_TYPE, getBoardDetails());
                        throw new TicTacToeError(`Cell (${r},${c}) is already ${gameState.board[r][c]}. Your cursor is still at (${r},${c}). Move to an empty cell and retry play.`);
                    }
    ...
    pi.registerTool({
        ...
        label: "See Board",
        description: "Return the current tic-tac-toe board state and YOUR cursor position (Player O). Takes no arguments. Use this if you need the current state restated mid-turn (for example after a failed play). The user's cursor is never exposed.",
        promptSnippet: "Inspect the tic-tac-toe board and your cursor",
        parameters: Type.Object({}),
        async execute(_toolCallId, _params, _signal, _onUpdate, _ctx) {
            const boardAscii = boardToAscii(gameState.board, gameState.agentCursorRow, gameState.agentCursorCol);
            const text = `Board (your cursor marked with <>):\n${boardAscii}\n\n` +
