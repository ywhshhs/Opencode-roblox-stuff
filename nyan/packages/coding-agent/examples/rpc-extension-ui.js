...
import { Container, Input, matchesKey, ProcessTerminal, SelectList, TUI } from "@nyan-works/nyan-tui";
...
class InputDialog {
    constructor(title, prefill) {
        this.title = title;
        this.dialogInput = new Input();
        if (prefill)
            this.dialogInput.setValue(prefill);
    }
    set onSubmit(fn) {
...
async function main() {
    ...
    const agent = spawn("node", [cliPath, "--mode", "rpc", "--no-session", "--no-extension", "--extension", extensionPath], { stdio: ["pipe", "pipe", "pipe"] });
    let stderr = "";
    ...
    function setBottomComponent(component) {
        root.clear();
        root.addChild(outputLog);
        if (isStreaming)
    ...
    function showSelectDialog(title, options, onDone) {
    ...
    function showInputDialog(title, prefill, onDone) {
    ...
    function handleExtensionUI(req) {
        ...
        switch (method) {
            ...
            case "setStatus":
                outputLog.append(`${MAGENTA}${BOLD}Notification:${RESET} ${DIM}[status: ${req.statusKey}]${RESET} ${req.statusText ?? "(cleared)"}`);
                tui.requestRender();
                break;
            case "setWidget": {
            ...
            case "set_editor_text":
                promptInput.input.setValue(req.text ?? "");
                tui.requestRender();
                break;
        }
    }
...
//# sourceMappingURL=rpc-extension-ui.js.map