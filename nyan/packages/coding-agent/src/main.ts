...
import { type ImageContent, modelsAreEqual } from "@nyan-works/nyan-ai";
...
import { main as goalMain } from "@nyan-works/nyan-goal/cli";
import { main as todoMain } from "@nyan-works/nyan-todo";
...
export async function main(args: string[], options?: MainOptions) {
	...
	// Handle /goal and /todo subcommands (new nyan-goal and nyan-todo packages)
	if (args[0] === "goal") {
		await goalMain();
		return;
	}
	...
	if (args[0] === "todo") {
		// Delegate to nyan-todo CLI — it reads from process.argv internally
		await todoMain();
		return;
	}
