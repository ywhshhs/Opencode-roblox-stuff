...
npm uninstall -g @nyan-works/nyan-coding-agent

# pnpm
pnpm remove -g @nyan-works/nyan-coding-agent

# Yarn
yarn global remove @nyan-works/nyan-coding-agent

# Bun
bun uninstall -g @nyan-works/nyan-coding-agent
```

Uninstalling pi leaves settings, credentials, sessions, and installed pi packages in `~/.nyan/agent/`.
...
You can also run `/login` and select an API-key provider to store the key in `~/.nyan/agent/auth.json`.
