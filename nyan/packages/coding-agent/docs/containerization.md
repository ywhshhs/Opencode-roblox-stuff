...
```bash
cp -R packages/coding-agent/examples/extensions/gondolin ~/.nyan/agent/extensions/gondolin
cd ~/.nyan/agent/extensions/gondolin
...
```bash
cd /path/to/project
pi -e ~/.nyan/agent/extensions/gondolin
```
...
RUN npm install -g --ignore-scripts @nyan-works/nyan-coding-agent
...
docker run --rm -it \
  -e ANTHROPIC_API_KEY \
  -v "$PWD:/workspace" \
  -v pi-agent-home:/root/.nyan/agent \
  pi-sandbox
```

The `-v "$PWD:/workspace"` mounts your current directory into the container at /workspace such that reads and writes in `/workspace` inside Docker directly affect your host files, like in the Gondolin example.

Use a named volume for `/root/.nyan/agent` if you want container-local settings and sessions. Mounting your host `~/.nyan/agent` exposes host auth and session files to the container.
