...
By default, `install` and `remove` write to user settings (`~/.nyan/agent/settings.json`). Use `-l` to write to project settings (`.nyan/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup after the project is trusted.
...
- User installs go under `~/.nyan/agent/npm/`.
- Project installs go under `.nyan/npm/`.
...
- Cloned to `~/.nyan/agent/git/<host>/<path>` (global) or `.nyan/git/<host>/<path>` (project).
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@nyan-works/nyan-ai`, `@nyan-works/nyan-agent-core`, `@nyan-works/nyan-coding-agent`, `@nyan-works/nyan-tui`, `typebox`.
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.nyan/agent`) and project (`.nyan/`) scopes.
