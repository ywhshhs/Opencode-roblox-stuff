...
> **Placement for /reload:** Put extensions in `~/.nyan/agent/extensions/` (global) or `.nyan/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.nyan/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.nyan/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.nyan/agent/extensions/*.ts` | Global (all projects) |
| `~/.nyan/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.nyan/extensions/*.ts` | Project-local |
| `.nyan/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@nyan-works/nyan-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@nyan-works/nyan-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@nyan-works/nyan-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@nyan-works/nyan-coding-agent";
...
```typescript
import { isBashToolResult } from "@nyan-works/nyan-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@nyan-works/nyan-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@nyan-works/nyan-coding-agent";
...
```typescript
import { SessionManager } from "@nyan-works/nyan-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@nyan-works/nyan-tui";
...
```typescript
import { withFileMutationQueue } from "@nyan-works/nyan-coding-agent";
...
**Important:** Use `StringEnum` from `@nyan-works/nyan-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@nyan-works/nyan-coding-agent";
...
} from "@nyan-works/nyan-coding-agent";
...
```typescript
import { keyHint } from "@nyan-works/nyan-coding-agent";
...
```typescript
import { Text, Component } from "@nyan-works/nyan-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@nyan-works/nyan-coding-agent";
import { matchesKey } from "@nyan-works/nyan-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@nyan-works/nyan-coding-agent";
