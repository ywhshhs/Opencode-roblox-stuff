...
```
~/.nyan/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.nyan/agent/sessions/`.
...
For TypeScript definitions in your project, inspect `node_modules/@nyan-works/nyan-coding-agent/dist/` and `node_modules/@nyan-works/nyan-ai/dist/`.
