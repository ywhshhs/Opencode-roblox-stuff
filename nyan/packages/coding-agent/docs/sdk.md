...
```bash
npm install @nyan-works/nyan-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@nyan-works/nyan-coding-agent";
...
The `Agent` class (from `@nyan-works/nyan-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.nyan/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.nyan/extensions/`)
- Project skills:
  - `.nyan/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.nyan/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.nyan/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@nyan-works/nyan-ai";
import { AuthStorage, ModelRegistry } from "@nyan-works/nyan-coding-agent";
...
// Default: uses ~/.nyan/agent/auth.json and ~/.nyan/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@nyan-works/nyan-coding-agent";
...
```typescript
import { createAgentSession } from "@nyan-works/nyan-coding-agent";
...
import { createAgentSession, defineTool } from "@nyan-works/nyan-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.nyan/agent/extensions/`, `.nyan/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@nyan-works/nyan-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@nyan-works/nyan-coding-agent";
...
1. Global: `~/.nyan/agent/settings.json`
2. Project: `<cwd>/.nyan/settings.json`
