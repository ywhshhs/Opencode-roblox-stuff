...
import { deleteKittyImage, encodeKitty, resetCapabilitiesCache, setCapabilities, setCellDimensions, } from "../src/terminal-image.js";
...
describe("TUI Kitty image cleanup", () => {
    ...
    it("falls back to full redraw when Kitty image pre-clear would scroll", async () => {
        ...
        try {
            ...
            terminal.clearWrites();
            const image = new Image("AAAA", "image/png", { fallbackColor: (value) => value }, { maxWidthCells: 3 }, { widthPx: 30, heightPx: 30 });
    ...
    it("does not use cursor-up placement for Kitty images taller than the viewport", async () => {
        ...
        try {
            ...
            await terminal.waitForRender();
            terminal.clearWrites();
            const image = new Image("AAAA", "image/png", { fallbackColor: (value) => value }, { maxWidthCells: 6 }, { widthPx: 60, heightPx: 60 });
            const imageLines = image.render(40);
            const imageSequence = imageLines[0];
