...
describe("Bug regression: isImageLine() crash with image escape sequences", () => {
    describe("Bug scenario: Terminal without image support", () => {
        it("old implementation would return false, causing crash", () => {
            ...
            const oldIsImageLine = (line, imageEscapePrefix) => {
            ...
            const lineWithImageSequence = "Read image file [image/jpeg]\x1b]1337;File=size=800,600;inline=1:base64data...\x07";
            ...
            assert.strictEqual(oldResult, false, "Bug: old implementation returns false for line containing image sequence when terminal has no image support");
        });
    ...
    describe("Integration: Tool execution scenario", () => {
        ...
        it("handles ANSI codes before image sequences", async () => {
            ...
            for (const line of lines) {
                assert.strictEqual(isImageLine(line), true, `Should detect image sequence after ANSI codes: ${line.slice(0, 30)}...`);
            }
        });
    });
...
//# sourceMappingURL=bug-regression-isimageline-startswith-bug.test.js.map