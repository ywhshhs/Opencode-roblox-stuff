...
const setupFolder = (baseDir, structure = {}) => {
...
const getSuggestions = (provider, lines, cursorLine, cursorCol, force = false) => provider.getSuggestions(lines, cursorLine, cursorCol, { signal: new AbortController().signal, force });
describe("CombinedAutocompleteProvider", () => {
    ...
    describe("fd @ file suggestions", { skip: !isFdInstalled }, () => {
        ...
        test("includes hidden paths but excludes .git", async () => {
            ...
            const result = await getSuggestions(provider, [line], 0, line.length);
            const values = result?.items.map((item) => item.value) ?? [];
            assert.ok(values.includes("@.nyan/"));
        ...
        test("returns the same @ suggestions when the cwd path contains the query", async () => {
            ...
            const normalize = (result) => (result?.items ?? []).map((item) => `${item.label} :: ${item.description ?? ""}`).sort();
