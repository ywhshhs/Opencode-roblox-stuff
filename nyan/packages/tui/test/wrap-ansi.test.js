    ...
    describe("background color preservation", () => {
        ...
        it("should reset underline but preserve background when wrapping underlined text inside background", () => {
            ...
            for (let i = 0; i < wrapped.length - 1; i++) {
                ...
                if ((line.includes("[4m") || line.includes("[4;") || line.includes(";4m")) &&
    ...
    it("re-emits OSC 8 open at the start of continuation lines", () => {
        ...
        for (const line of lines) {
            ...
            if (stripped.trim().length > 0) {
                assert.ok(line.startsWith(`\x1b]8;;${url}\x1b\\`) || line.includes(`\x1b]8;;${url}\x1b\\`), `Line "${line}" has visible text but no OSC 8 re-open`);
            }
        }
    });
    it("closes OSC 8 before each line break", () => {
        ...
        for (let i = 0; i < lines.length - 1; i++) {
            ...
            if (line.includes(`\x1b]8;;${url}\x1b\\`)) {
                assert.ok(line.endsWith("\x1b]8;;\x1b\\"), `Non-final line "${line}" is inside a hyperlink but does not close it`);
            }
        }
    });
...
//# sourceMappingURL=wrap-ansi.test.js.map