import { fauxAssistantMessage, fauxText, fauxThinking, fauxToolCall, registerFauxProvider, } from "@nyan-works/nyan-ai/compat";
import { afterEach, describe, expect, it } from "vitest";
import { Agent } from "../src/index.js";
import { calculateTool } from "./utils/calculate.js";
const registrations = [];
function createFauxRegistration(options = {}) {
...
async function basicPrompt(model) {
...
async function toolExecution(model) {
    const agent = new Agent({
        initialState: {
    ...
    const pendingToolCallsDuringEvents = [];
...
async function abortExecution(model) {
    const agent = new Agent({
        initialState: {
...
async function stateUpdates(model) {
    const agent = new Agent({
        initialState: {
            systemPrompt: "You are a helpful assistant.",
            model,
...
async function multiTurnConversation(model) {
    const agent = new Agent({
        initialState: {
            systemPrompt: "You are a helpful assistant.",
            model,
            ...
            (context) => {
                const hasAlice = context.messages.some((message) => {
                    ...
                    if (typeof message.content === "string")
                        return message.content.includes("Alice");
