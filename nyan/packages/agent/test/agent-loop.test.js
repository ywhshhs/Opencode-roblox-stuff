import { EventStream, } from "@nyan-works/nyan-ai";
...
describe("agentLoop with AgentMessage", () => {
    ...
    it("should handle custom message types via convertToLlm", async () => {
        ...
        const context = {
            systemPrompt: "You are helpful.",
            messages: [notification], // Custom message in context
            tools: [],
        };
        const userPrompt = createUserMessage("Hello");
    ...
    it("should apply transformContext before convertToLlm", async () => {
        ...
        const config = {
            ...
            convertToLlm: (messages) => {
                convertedMessages = messages.filter((m) => m.role === "user" || m.role === "assistant" || m.role === "toolResult");
                return convertedMessages;
            },
        };
        const streamFn = () => {
        ...
        const streamFn = () => {
            ...
            queueMicrotask(() => {
                if (callIndex === 0) {
                    ...
                    const message = createAssistantMessage([{ type: "toolCall", id: "tool-1", name: "echo", arguments: { value: "hello" } }], "toolUse");
    ...
    it("should prepare tool arguments for validation", async () => {
        ...
        const userPrompt = createUserMessage("edit something");
        const config = {
            model: createModel(),
            convertToLlm: identityConverter,
        };
    ...
    it("should inject queued messages after all tool calls complete", async () => {
        ...
        const stream = agentLoop([userPrompt], context, config, undefined, (_model, ctx, _options) => {
            ...
            if (callIndex === 1) {
                sawInterruptInContext = ctx.messages.some((m) => m.role === "user" && typeof m.content === "string" && m.content === "interrupt");
            }
            const mockStream = new MockAssistantStream();
            queueMicrotask(() => {
                if (callIndex === 0) {
            ...
            queueMicrotask(() => {
                if (llmCalls === 1) {
                    mockStream.push({
                        type: "done",
                        reason: "toolUse",
                        message: createAssistantMessage([{ type: "toolCall", id: "tool-1", name: "echo", arguments: { value: "hello" } }], "toolUse"),
                    });
                }
                else {
                    mockStream.push({
    ...
    it("should stop after the current turn when shouldStopAfterTurn returns true", async () => {
        ...
        let steeringPolls = 0;
        let followUpPolls = 0;
        let callbackToolResultIds = [];
        let callbackContextRoles = [];
        const config = {
            model: createModel(),
            convertToLlm: identityConverter,
            getSteeringMessages: async () => {
