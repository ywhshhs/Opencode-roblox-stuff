...
describe("loadSkills", () => {
    it("loads SKILL.md files through the execution environment", async () => {
        ...
        await env.writeFile(".agents/skills/example/SKILL.md", `---
name: example
...
`);
    ...
    it("loads skills through symlinked directories", async () => {
        ...
        await env.writeFile("actual/example/SKILL.md", "---\nname: example\ndescription: Example skill\n---\nUse this skill.");
    ...
    it("preserves source info for sourced skills", async () => {
        ...
        await env.writeFile("user/example/SKILL.md", "---\nname: example\ndescription: Example skill\n---\nUse this skill.");
        const { skills, diagnostics } = await loadSourcedSkills(env, [
            { path: "user", source: { type: "user" } },
        ]);
        expect(diagnostics).toEqual([]);
        expect(skills).toEqual([
            {
