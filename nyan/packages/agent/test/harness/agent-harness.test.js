import { createModels, fauxAssistantMessage, fauxProvider, fauxToolCall, } from "@nyan-works/nyan-ai";
...
let fauxCount = 0;
function newFaux(options = {}) {
    const faux = fauxProvider({ provider: `faux-${++fauxCount}`, ...options });
    models.setProvider(faux.provider);
    return faux;
}
function textFromUserMessages(messages) {
...
describe("AgentHarness", () => {
    ...
    it("refreshes model, thinking level, resources, system prompt, and active tools at save points", async () => {
        ...
        if (!secondModel)
            throw new Error("missing second faux model");
        const captured = [];
        registration.setResponses([
    ...
    it("runs tool_call and tool_result hooks through the direct loop", async () => {
        ...
        await harness.prompt("hello");
        const toolResult = (await session.getEntries()).find((entry) => entry.type === "message" && entry.message.role === "toolResult");
    ...
    it("preserves app tool types for getters and update events", async () => {
        ...
        const inspectTool = { ...calculateTool, name: "inspect", source: "builtin" };
        const searchTool = { ...calculateTool, name: "search", source: "extension" };
        const harness = new AgentHarness({
            models,
            env,
            session,
    ...
    it("validates constructor tool names", () => {
        ...
        const env = new NodeExecutionEnv({ cwd: process.cwd() });
        const model = getModel("anthropic", "claude-sonnet-4-5");
        expect(() => new AgentHarness({ env, session, models, model, tools: [calculateTool], activeToolNames: ["missing"] })).toThrow(/Unknown tool/);
        expect(() => new AgentHarness({
            models,
            env,
            session,
            model,
    ...
    it("preserves app resource types for getters and update events", async () => {
        ...
        const promptTemplate = { name: "review", content: "Review $1", source: "user" };
...
//# sourceMappingURL=agent-harness.test.js.map