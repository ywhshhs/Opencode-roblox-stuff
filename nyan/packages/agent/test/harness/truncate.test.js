...
function bufferTail(content, maxBytes) {
...
function assertMatchesBufferTail(input, maxByteValues) {
    ...
    for (const maxBytes of values) {
        ...
        if (result.content !== expected) {
            throw new Error(`tail mismatch input=${JSON.stringify(input)} maxBytes=${maxBytes} expected=${JSON.stringify(expected)} actual=${JSON.stringify(result.content)}`);
        }
        const outputBytes = Buffer.byteLength(result.content, "utf8");
        if (outputBytes > maxBytes) {
            throw new Error(`tail output exceeded byte limit input=${JSON.stringify(input)} maxBytes=${maxBytes} outputBytes=${outputBytes}`);
        }
    }
}
function sampledByteLimits(input) {
    const totalBytes = Buffer.byteLength(input, "utf8");
    const candidates = [
        0,
        1,
...
describe("truncate utilities", () => {
    ...
    it("matches Buffer tail truncation semantics across deterministic fuzz cases", () => {
        ...
        function checkExhaustive(prefix, depth) {
