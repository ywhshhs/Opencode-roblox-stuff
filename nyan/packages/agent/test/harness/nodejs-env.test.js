...
describe("NodeExecutionEnv", () => {
    ...
    it("lists symlinks as symlinks", async () => {
        ...
        expect(entries.map((entry) => ({ name: entry.name, kind: entry.kind })).sort((a, b) => a.name.localeCompare(b.name))).toEqual([
    ...
    it("honors createDir recursive false and remove recursive/force options", async () => {
        ...
        if (!createResult.ok)
            expect(createResult.error).toMatchObject({ code: "not_found" });
    ...
    it("executes commands in cwd with env overrides", async () => {
        const root = createTempDir();
        const env = new NodeExecutionEnv({ cwd: root });
        const result = getOrThrow(await env.exec('printf \'%s:%s\' "$PWD" "$NODE_ENV_TEST"', {
    ...
    it("streams stdout and stderr chunks", async () => {
        ...
        let stdout = "";
        let stderr = "";
        const result = getOrThrow(await env.exec("printf out; printf err >&2", {
            onStdout: (chunk) => {
                stdout += chunk;
            },
            onStderr: (chunk) => {
    ...
    it("returns shell unavailable and spawn errors", async () => {
        ...
        if (!missingShell.ok)
            expect(missingShell.error).toMatchObject({ code: "shell_unavailable" });
        ...
        if (!spawnError.ok)
            expect(spawnError.error).toMatchObject({ code: "spawn_error" });
    });
...
//# sourceMappingURL=nodejs-env.test.js.map