import { createModels, fauxAssistantMessage, fauxProvider, } from "@nyan-works/nyan-ai";
import { beforeEach, describe, expect, it } from "vitest";
import { calculateContextTokens, compact, DEFAULT_COMPACTION_SETTINGS, estimateContextTokens, estimateTokens, findCutPoint, findTurnStartIndex, generateSummary, getLastAssistantUsage, prepareCompaction, serializeConversation, shouldCompact, } from "../../src/harness/compaction/compaction.js";
...
function createMockUsage(input, output, cacheRead = 0, cacheWrite = 0) {
    return {
        input,
        output,
        cacheRead,
...
function createAssistantMessage(text, usage = createMockUsage(100, 50)) {
    return {
        role: "assistant",
        content: [{ type: "text", text }],
...
function createMessageEntry(message, parentId = null) {
    return {
        type: "message",
        id: createId(),
        parentId,
...
function createCompactionEntry(summary, firstKeptEntryId, parentId = null) {
    return {
        type: "compaction",
        id: createId(),
        parentId,
...
function createThinkingLevelEntry(level, parentId = null) {
    return {
...
function createModelChangeEntry(provider, modelId, parentId = null) {
    return {
        type: "model_change",
        id: createId(),
        parentId,
...
let fauxCount = 0;
function createFauxModel(reasoning, maxTokens = 8192) {
...
describe("harness compaction", () => {
    ...
    it("finds a cut point based on token differences", () => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            entries.push(user);
            const assistant = createMessageEntry(createAssistantMessage(`Assistant ${i}`, createMockUsage(0, 100, (i + 1) * 1000, 0)), user.id);
    ...
    it("estimates tokens and context usage across supported message roles", () => {
        ...
        const assistantWithThinkingAndTool = {
            ...assistant,
            content: [
        ...
        expect(estimateTokens({ role: "unknown", timestamp: Date.now() })).toBe(0);
        expect(getLastAssistantUsage([createMessageEntry(createUserMessage("user")), createMessageEntry(assistant)])).toBe(usage);
        expect(getLastAssistantUsage([
    ...
    it("prepares split-turn compaction with prior file-operation details", () => {
        ...
        const preparation = getOrThrow(prepareCompaction([u1, a1, compaction1, u2, a2], {
            enabled: true,
            reserveTokens: 100,
            keepRecentTokens: 1,
        }));
    ...
    it("prepares custom and branch summary entries for summarization", () => {
        ...
        const preparation = getOrThrow(prepareCompaction([branchSummary, customMessage, user, assistant], {
            enabled: true,
            reserveTokens: 100,
            keepRecentTokens: 1,
        }));
    ...
    it("passes reasoning through generateSummary only for reasoning models with thinking enabled", async () => {
        const messages = [createUserMessage("Summarize this.")];
        const seenOptions = [];
        ...
        getOrThrow(await generateSummary(messages, models, reasoningModel, 2000, undefined, undefined, undefined, "medium"));
        ...
        getOrThrow(await generateSummary(messages, models, nonReasoningModel, 2000, undefined, undefined, undefined, "medium"));
    ...
    it("includes previous summaries and custom instructions in generateSummary prompts", async () => {
        ...
        const summary = getOrThrow(await generateSummary(messages, models, model, 2000, undefined, "focus", "old summary"));
    ...
    it("returns compaction error results without throwing", async () => {
        ...
        const invalidResult = await compact({ ...preparation, messagesToSummarize: [], firstKeptEntryId: "" }, models, invalidModel);
