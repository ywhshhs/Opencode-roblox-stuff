...
const packages = [
	{ directory: "packages/ai", name: "@nyan-works/nyan-ai" },
	{ directory: "packages/tui", name: "@nyan-works/nyan-tui" },
	{ directory: "packages/agent", name: "@nyan-works/nyan-agent-core" },
	{ directory: "packages/coding-agent", name: "@nyan-works/nyan-coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "nyan-local-release-"));
	}
...
if (rootPackageJson.name !== "nyan-monorepo") {
