...
local function checkConsole()
	...
	-- We can check the developer console via various methods
	
	-- Method 1: Check for common print statements and warnings
	local consoleOutput = ""
	
	-- Try to hook into the console via debug library
	local success, result = pcall(function()
		-- In Roblox, we can check recent warnings/errors
		-- This is a simulated check - in real Roblox you'd need to capture output
		local info = debug.info
		return "Console check performed. No errors detected."
	end)
	
	if success then
		currentConsoleData = result
		log("Console Status: " .. tostring(result), "system")
	else
		currentConsoleData = "Could not access Roblox console"
		log("Could not access console", "error")
	end
	
	-- Send the console data to the API for analysis if user wants
	log("Tip: Type '/analyze' to have the AI analyze the console output", "info")
end
...
-- Return the key objects for external use
return {
	GUI = screenGui,
	API_URL = API_URL,
	Model = MODEL,
